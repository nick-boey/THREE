...
{
    private readonly CffEvaluationEngine _cffEvalEngine;
    ...
    {
        Typeface = typeface;
        UseTrueTypeInstructions = true; //default?
        _recentPixelScale = 1;

        if (typeface.IsCffFont) _cffEvalEngine = new CffEvaluationEngine();
    }
    ...
    public void BuildFromGlyph(Glyph glyph, float sizeInPoints)
    {
        ...
        if (glyph.IsCffGlyph) _cffGlyphData = glyph.GetCff1GlyphData();
...
public static class GlyphPathBuilderExtensions
{
    ...
    {
        builder.BuildFromGlyphIndex(builder.Typeface.GetGlyphIndex(c), sizeInPoints);
    }
