...
public class Mesh
{
...
#if DEBUG
    private static int s_dbugFaceIndexTotal;
#endif

    private readonly HalfEdge _otherHalfOfThisEdgeHead = new(); /* and its symmetric counterpart */
    internal Face _faceHead = new(); /* dummy header for face list */
    internal HalfEdge _halfEdgeHead = new(); /* dummy header for edge list */

    internal ContourVertex _vertexHead = new(); /* dummy header for vertex list */
    ...
    public Mesh()
    {
        var otherHalfOfThisEdge = _otherHalfOfThisEdgeHead;
    ...
    {
        var aOnext = a._nextEdgeCCWAroundOrigin;
        var bOnext = b._nextEdgeCCWAroundOrigin;
    ...
    private static void KillEdge(HalfEdge eDel)
    {
        ...
        if (eDel._otherHalfOfThisEdge._isFirstHalfEdge) eDel = eDel._otherHalfOfThisEdge;
    ...
    {
        var otherHalfOfEdgeToDelete = edgeToDelete._otherHalfOfThisEdge;
        var joiningLoops = false;
        ...
        else
        {
            ...
            otherHalfOfEdgeToDelete._originVertex._edgeThisIsOriginOf =
                otherHalfOfEdgeToDelete._nextEdgeCCWAroundOrigin;
    ...
    {
        HalfEdge eNew;
        var tempHalfEdge = meshAddEdgeVertex(eOrg);
    ...
    private static HalfEdge MakeEdge(HalfEdge eNext)
    {
        ...
        if (eNext._otherHalfOfThisEdge._isFirstHalfEdge) eNext = eNext._otherHalfOfThisEdge;
    ...
    {
        var f1 = mesh1._faceHead;
        var v1 = mesh1._vertexHead;
        var e1 = mesh1._halfEdgeHead;
        var f2 = mesh2._faceHead;
        var v2 = mesh2._vertexHead;
        var e2 = mesh2._halfEdgeHead;
    ...
    {
        var eStart = fZap._halfEdgeThisIsLeftFaceOf;
    ...
    */
    ...
    {
        var fHead = _faceHead;
        var vHead = _vertexHead;
        var eHead = _halfEdgeHead;
        Face f, fPrev;
        ...
        {
            if (f._prevFace != fPrev) throw new Exception();
            e = f._halfEdgeThisIsLeftFaceOf;
            do
            {
                if (e._otherHalfOfThisEdge == e) throw new Exception();
                if (e._otherHalfOfThisEdge._otherHalfOfThisEdge != e) throw new Exception();
                ...
                if (e._nextEdgeCCWAroundOrigin._otherHalfOfThisEdge._nextEdgeCCWAroundLeftFace != e)
                    throw new Exception();
                if (e._leftFace != f) throw new Exception();
                e = e._nextEdgeCCWAroundLeftFace;
            } while (e != f._halfEdgeThisIsLeftFaceOf);
        }

        if (f._prevFace != fPrev || f._halfEdgeThisIsLeftFaceOf != null) throw new Exception();

        vPrev = vHead;
        for (vPrev = vHead; (v = vPrev._nextVertex) != vHead; vPrev = v)
        {
            if (v._prevVertex != vPrev) throw new Exception();
            ...
            do
            {
                ...
                if (e._nextEdgeCCWAroundOrigin._otherHalfOfThisEdge._nextEdgeCCWAroundLeftFace != e)
                    throw new Exception();
                if (e._originVertex != v) throw new Exception();
                e = e._nextEdgeCCWAroundOrigin;
            } while (e != v._edgeThisIsOriginOf);
        }

        if (v._prevVertex != vPrev || v._edgeThisIsOriginOf != null || v._clientIndex != 0) throw new Exception();

        ePrev = eHead;
        for (ePrev = eHead; (e = ePrev._nextHalfEdge) != eHead; ePrev = e)
        {
            if (e._otherHalfOfThisEdge._nextHalfEdge != ePrev._otherHalfOfThisEdge) throw new Exception();
            if (e._otherHalfOfThisEdge == e) throw new Exception();
            if (e._otherHalfOfThisEdge._otherHalfOfThisEdge != e) throw new Exception();
            if (e._originVertex == null) throw new Exception();
            if (e.DirectionVertex == null) throw new Exception();
            if (e._nextEdgeCCWAroundLeftFace._nextEdgeCCWAroundOrigin._otherHalfOfThisEdge != e) throw new Exception();
            if (e._nextEdgeCCWAroundOrigin._otherHalfOfThisEdge._nextEdgeCCWAroundLeftFace != e) throw new Exception();
        }
    ...
    public void DiscardExterior()
    {
        ...
        for (f = _faceHead._nextFace; f != _faceHead; f = next)
        {
            ...
            next = f._nextFace;
            if (!f._isInterior) meshZapFace(f);
        }
    }
...
#endif
        public readonly HalfEdge e = new();
        public readonly HalfEdge eSym = new();
