...
public class ContourVertex : IComparable<ContourVertex>
{
    ...
    {
        if (VertEq(otherVertex)) return 0;
        if (VertLeq(otherVertex)) return -1;
        return 1;
    }
    ...
    public static double EdgeEval(ContourVertex u, ContourVertex v, ContourVertex w)
    {
         ...
         */
        double gapL, gapR;
        if (!(u.VertLeq(u) && v.VertLeq(v))) throw new Exception();

        gapL = v.x - u.x;
        gapR = w.x - v.x;
        if (gapL + gapR > 0)
        {
            if (gapL < gapR) return v.y - u.y + (u.y - w.y) * (gapL / (gapL + gapR));
    ...
    public static double EdgeSign(ContourVertex u, ContourVertex v, ContourVertex w)
    {
         ...
         */
        double gapL, gapR;
        if (!u.VertLeq(v) || !v.VertLeq(w)) throw new Exception();

        gapL = v.x - u.x;
        gapR = w.x - v.x;
        if (gapL + gapR > 0) return (v.y - w.y) * gapL + (v.y - u.y) * gapR;
        /* vertical line */
        return 0;
    }
