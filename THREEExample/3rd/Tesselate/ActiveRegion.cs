...
public class ActiveRegion
{
         ...
         */
    {
        ...
        DoneEdgeDict(tess);
        DonePriorityQ(tess);
        if (!RemoveDegenerateFaces(tess._mesh)) return 0;
    ...
    public static bool EdgeLeq(Tesselator tess, ActiveRegion reg1, ActiveRegion reg2)
         ...
         */
    {
        var currentSweepVertex = tess.currentSweepVertex;
        HalfEdge e1, e2;
        double t1, t2;
    ...
    private static bool FixUpperEdge(ActiveRegion reg, HalfEdge newEdge)
         ...
         */
    {
        if (!reg._fixUpperEdge) throw new Exception();
    ...
    {
        var org = reg._upperHalfEdge._originVertex;
        HalfEdge e;
        ...
        if (reg._fixUpperEdge)
        {
            e = Mesh.meshConnect(RegionBelow(reg)._upperHalfEdge._otherHalfOfThisEdge,
                reg._upperHalfEdge._nextEdgeCCWAroundLeftFace);
    ...
    {
        var dst = reg._upperHalfEdge.DirectionVertex;
         ...
         */
    {
        ...
        do
        {
            if (!e._originVertex.VertLeq(e.DirectionVertex)) throw new Exception();
         ...
         */
        if (eTopLeft == null) eTopLeft = RegionBelow(regUp)._upperHalfEdge.Rprev;
        regPrev = regUp;
        ePrev = eTopLeft;
        for (;;)
        {
        ...
        regPrev._dirty = true;
        if (regPrev._windingNumber - e._winding != reg._windingNumber) throw new Exception();
    ...
    private static void CallCombine(Tesselator tess, ContourVertex intersectionVertex,
        ref Tesselator.CombineParameters combinePars, bool needed)
    {
        /* Copy coord data in case the callback changes it. */
        var c0 = intersectionVertex._C_0;
        var c1 = intersectionVertex._C_1;
        var c2 = intersectionVertex._C_2;
    ...
    private static void VertexWeights(ContourVertex isect, ContourVertex org, ContourVertex dst, out double weights0,
         ...
         */
    {
        var t1 = VertL1dist(org, isect);
        var t2 = VertL1dist(dst, isect);
         ...
         */
    {
        ...
        var eUp = regUp._upperHalfEdge;
        var eLo = regLo._upperHalfEdge;
        if (eUp._originVertex.VertLeq(eLo._originVertex))
        {
            if (ContourVertex.EdgeSign(eLo.DirectionVertex, eUp._originVertex, eLo._originVertex) > 0) return false;
        ...
        {
            if (ContourVertex.EdgeSign(eUp.DirectionVertex, eLo._originVertex, eUp._originVertex) < 0) return false;
         ...
         */
    {
        ...
        var eLo = regLo._upperHalfEdge;
        HalfEdge e;
        if (eUp.DirectionVertex.VertEq(eLo.DirectionVertex)) throw new Exception();

        if (eUp.DirectionVertex.VertLeq(eLo.DirectionVertex))
        {
            if (ContourVertex.EdgeSign(eUp.DirectionVertex, eLo.DirectionVertex, eUp._originVertex) < 0) return false;
    ...
    private static double TransEval(ContourVertex u, ContourVertex v, ContourVertex w)
    {
         ...
         */
        double gapL, gapR;
        if (!u.TransLeq(v) || !v.TransLeq(w)) throw new Exception();

        gapL = v.y - u.y;
        gapR = w.y - v.y;
        if (gapL + gapR > 0)
        {
            if (gapL < gapR) return v.x - u.x + (u.x - w.x) * (gapL / (gapL + gapR));
    ...
    private static double TransSign(ContourVertex u, ContourVertex v, ContourVertex w)
    {
        ...
        gapL = v.y - u.y;
        gapR = w.y - v.y;
        if (gapL + gapR > 0) return (v.x - w.x) * gapL + (v.x - u.x) * gapR;
        /* vertical line */
        return 0;
    }
         ...
         */
    {
        ...
        var eUp = regUp._upperHalfEdge;
        var eLo = regLo._upperHalfEdge;
        var orgUp = eUp._originVertex;
        var orgLo = eLo._originVertex;
        var dstUp = eUp.DirectionVertex;
        var dstLo = eLo.DirectionVertex;
        double tMinUp, tMaxLo;
        var isect = new ContourVertex();
        ContourVertex orgMin;
        HalfEdge e;
        if (dstLo.VertEq(dstUp)) throw new Exception();
        if (ContourVertex.EdgeSign(dstUp, tess.currentSweepVertex, orgUp) > 0) throw new Exception();
        if (ContourVertex.EdgeSign(dstLo, tess.currentSweepVertex, orgLo) < 0) throw new Exception();
        if (orgUp == tess.currentSweepVertex || orgLo == tess.currentSweepVertex) throw new Exception();
        if (regUp._fixUpperEdge || regLo._fixUpperEdge) throw new Exception();
        ...
        {
            if (ContourVertex.EdgeSign(dstLo, orgUp, orgLo) > 0) return false;
        }
        else
        {
            if (ContourVertex.EdgeSign(dstUp, orgLo, orgUp) < 0) return false;
        }

        EdgeIntersect(dstUp, orgUp, dstLo, orgLo, ref isect);
        // The following properties are guaranteed:
        if (!(Math.Min(orgUp.y, dstUp.y) <= isect.y)) throw new Exception();
        if (!(isect.y <= Math.Max(orgLo.y, dstLo.y))) throw new Exception();
        if (!(Math.Min(dstLo.x, dstUp.x) <= isect.x)) throw new Exception();
        if (!(isect.x <= Math.Max(orgLo.x, orgUp.x))) throw new Exception();
        ...
        tess._vertexPriorityQue.Add(out eUp._originVertex._priorityQueueHandle,
            eUp._originVertex); /* __gl_pqSortInsert */
    ...
    {
        HalfEdge eNew;
        var eTopLeft = eBottomLeft._nextEdgeCCWAroundOrigin;
        var regLo = RegionBelow(regUp);
        var eUp = regUp._upperHalfEdge;
        var eLo = regLo._upperHalfEdge;
        var degenerate = false;
        if (eUp.DirectionVertex != eLo.DirectionVertex) CheckForIntersect(tess, regUp);
         ...
         */
    {
        ...
        if (reg._fixUpperEdge)
        {
             ...
             */
            if (eTopLeft == eTopRight) throw new Exception(); /* there are some left edges too */
            DeleteRegion(reg);
         ...
         */
    {
        ...
        if (regUp._inside || reg._fixUpperEdge)
        {
            if (reg == regUp)
            {
                eNew = Mesh.meshConnect(vEvent._edgeThisIsOriginOf._otherHalfOfThisEdge,
                    eUp._nextEdgeCCWAroundLeftFace);
            }
            else
            {
                var tempHalfEdge = Mesh.meshConnect(eLo.Dnext, vEvent._edgeThisIsOriginOf);
    ...
    private static void DoneEdgeDict(Tesselator tess)
    {
        ...
        while ((reg = tess._edgeDictionary.GetMinNode().Key) != null)
        {
            ...
            {
                if (!reg._fixUpperEdge) throw new Exception();
                if (++fixedEdges != 1) throw new Exception();
            }

            if (reg._windingNumber != 0) throw new Exception();
            DeleteRegion(reg);
        }
    ...
    private static void RemoveDegenerateEdges(Tesselator tess)
    {
        ...
        var edgeHead = tess._mesh._halfEdgeHead;
        HalfEdge nextHalfEdge;
        for (var currentEdge = edgeHead._nextHalfEdge; currentEdge != edgeHead; currentEdge = nextHalfEdge)
        {
            nextHalfEdge = currentEdge._nextHalfEdge;
            var nextEdgeCCWAroundLeftFace = currentEdge._nextEdgeCCWAroundLeftFace;
            ...
            if (nextEdgeCCWAroundLeftFace._nextEdgeCCWAroundLeftFace == currentEdge)
            {
                ...
                if (nextEdgeCCWAroundLeftFace != currentEdge)
                {
                    if (nextEdgeCCWAroundLeftFace == nextHalfEdge ||
                        nextEdgeCCWAroundLeftFace == nextHalfEdge._otherHalfOfThisEdge)
                        nextHalfEdge = nextHalfEdge._nextHalfEdge;

                    Mesh.DeleteHalfEdge(nextEdgeCCWAroundLeftFace);
                }

                if (currentEdge == nextHalfEdge || currentEdge == nextHalfEdge._otherHalfOfThisEdge)
                    nextHalfEdge = nextHalfEdge._nextHalfEdge;
    ...
    private static void InitPriorityQue(Tesselator tess)
         ...
         */
    {
        var priorityQue = tess._vertexPriorityQue = new MaxFirstList<ContourVertex>();
        var vertexHead = tess._mesh._vertexHead;
        for (var curVertex = vertexHead._nextVertex; curVertex != vertexHead; curVertex = curVertex._nextVertex)
         ...
         */
    {
        ...
        for (f = mesh._faceHead._nextFace; f != mesh._faceHead; f = fNext)
        {
            ...
            if (e._nextEdgeCCWAroundLeftFace == e) throw new Exception();
