...
public class Face
{
    ...
    internal bool TessellateMonoRegion()
    {
         ...
         */
        var up = _halfEdgeThisIsLeftFaceOf;
        ...
        while (up._nextEdgeCCWAroundLeftFace != lo)
            if (up.DirectionVertex.VertLeq(lo._originVertex))
            {
                ...
                while (lo._nextEdgeCCWAroundLeftFace != up && (lo._nextEdgeCCWAroundLeftFace.EdgeGoesLeft()
                                                               || ContourVertex.EdgeSign(lo._originVertex,
                                                                   lo.DirectionVertex,
                                                                   lo._nextEdgeCCWAroundLeftFace.DirectionVertex) <= 0))
                {
                    var tempHalfEdge = Mesh.meshConnect(lo._nextEdgeCCWAroundLeftFace, lo);
            ...
            else
            {
                ...
                while (lo._nextEdgeCCWAroundLeftFace != up && (up.Lprev.EdgeGoesRight()
                                                               || ContourVertex.EdgeSign(up.DirectionVertex,
                                                                   up._originVertex, up.Lprev._originVertex) >= 0))
                {
                    var tempHalfEdge = Mesh.meshConnect(up, up.Lprev);
        ...
        if (lo._nextEdgeCCWAroundLeftFace == up) throw new Exception();
