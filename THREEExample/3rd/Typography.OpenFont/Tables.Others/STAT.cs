...
internal class STAT : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        var offsetToAxisValueOffsets = reader.ReadUInt32();
        //
        var elidedFallbackNameID = minorVersion != 0 ? reader.ReadUInt16() : (ushort)0;
        ...
        var axisValueOffsets_beginPos = reader.BaseStream.Position = beginPos + offsetToAxisValueOffsets;
        var axisValueOffsets =
            reader.ReadUInt16Array(
                axisValueCount); // Array of offsets to axis value tables,in bytes from the start of the axis value offsets array.


        //move to axis value record

        var axisValueTables = new AxisValueTableBase[axisValueCount];
        for (var i = 0; i < axisValueCount; ++i)
        {
            ...
            var offset = axisValueOffsets[i];
            reader.BaseStream.Position = axisValueOffsets_beginPos + offset;

            var format = reader.ReadUInt16(); //common field of all axis value table
