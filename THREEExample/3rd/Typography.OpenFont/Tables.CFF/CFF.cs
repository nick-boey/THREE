...
internal class Cff1FontSet
{
    ...
    internal List<Cff1Font> _fonts = new();
...
{
    private readonly Type2InstructionCompacter _instCompacter = new();

    private readonly StringBuilder _sbForReadRealNumber = new();

    //---------------
    private readonly bool _useCompactInstruction = true;
    ...
    private void ReadNameIndex()
    {
        ...
        var fontNames = new string[count];
        ...
        //
        ResultCff1FontSet._fontNames = fontNames;
        ...
        ResultCff1FontSet._fonts.Add(_currentCff1Font);
    }
    ...
    private void ReadTopDICTIndex()
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            var dicData = ReadDICTData(offsets[i].len);
            _topDic = dicData;
        }
    }
    ...
    private void ReadStringIndex()
    {
        ...
        {
            var len = offsets[i].len;
            //TODO: review here again, 
            //check if we need to set _reader.BaseStream.Position or not 
            //TODO: Is Charsets.ISO_8859_1 Encoding supported in .netcore  
            if (len < buff.Length)
            {
                var actualRead = _reader.Read(buff, 0, len);
#if DEBUG
                if (actualRead != len) throw new NotSupportedException();
#endif
        ...
        ResultCff1FontSet._uniqueStringTable = _uniqueStringTable;
    }
    ...
    private void ReadCharsetsFormat0()
    {
        ...
        {
            var d = cff1Glyphs[i]._cff1GlyphData;
    ...
    private void ReadCharsetsFormat1()
    {
        ...
        for (var i = 1; i < nGlyphs;)
        {
            ...
            var count = _reader.ReadByte() + 1; //since it not include first elem
            do
            {
                var d = cff1Glyphs[i]._cff1GlyphData;
    ...
    private void ReadCharsetsFormat2()
    {
        ...
        {
            int sid = _reader.ReadUInt16(); // First glyph in range 
            var count = _reader.ReadUInt16() + 1; //since it not include first elem
            do
            {
                var d = cff1Glyphs[i]._cff1GlyphData;
                d.Name = GetSid(d.SIDName = (ushort)sid);
    ...
    private void ReadFDSelect()
    {
        ...
        switch (format)
        {
            ...
            case 3:
            {
                var nRanges = _reader.ReadUInt16();
                var ranges = new FDRange3[nRanges + 1];

                _cidFontInfo.fdSelectFormat = 3;
                _cidFontInfo.fdRanges = ranges;
                for (var i = 0; i < nRanges; ++i) ranges[i] = new FDRange3(_reader.ReadUInt16(), _reader.ReadByte());
    ...
    private void ReadFDArray()
    {
        ...
        {
            //read DICT data 
            var dic = ReadDICTData(offsets[i].len);
            //translate
        ...
        foreach (var fdict in fontDicts)
        {
            ...
            var dicData = ReadDICTData(fdict.PrivateDicSize);
    ...
    private void ReadCharStringsIndex()
    {
        ...
        var isCidFont = _cidFontInfo.fdRanges != null;
...
#if DEBUG
            //check
            var lastByte = buffer[offset.len - 1];
    ...
    private void ReadPrivateDict()
    {
        ...
        var dicData = ReadDICTData(_privateDICTLen);
    ...
    private List<byte[]> ReadSubrBuffer()
    {
        ...
        //
        var nsubrs = offsets.Length;
        var rawBufferList = new List<byte[]>();
    ...
    {
        if (b0 == 28) return _reader.ReadInt16();

        if (b0 == 29) return _reader.ReadInt32();
    ...
    private CffIndexOffset[] ReadIndexDataOffsets()
    {
        ...
        for (var i = 0; i <= count; ++i) offsets[i] = _reader.ReadOffset(offSize);
    ...
    private struct FDRangeProvider
    {
        ...
        private readonly FDRange3[] _ranges;
        ...
        public void SetCurrentGlyphIndex(ushort index)
        {
            ...
            {
                //ok, in current range
                SelectedFDArray = _currentRange.fd;
            }
            else
            {
                //move to next range
...
internal static class CFFBinaryReaderExtension
{
    public static int ReadOffset(this BinaryReader reader, int offsetSize)
    {
        switch (offsetSize)
        {
            ...
            case 4:
                return (reader.ReadByte() << 24) | (reader.ReadByte() << 16) | (reader.ReadByte() << 8) |
                       (reader.ReadByte() << 0);
        }
    }
}
...
{
    private static readonly Dictionary<int, CFFOperator> s_registered_Operators = new();
    ...
    static CFFOperator()
    {
        ...
        Register(12, 17, "LanguageGroup",
            OperatorOperandKind.Number); //https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
        Register(12, 18, "ExpansionFactor",
            OperatorOperandKind.Number); //https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
        Register(12, 19, "initialRandomSeed",
            OperatorOperandKind.Number); //https://typekit.files.wordpress.com/2013/05/5176.cff.pdf
