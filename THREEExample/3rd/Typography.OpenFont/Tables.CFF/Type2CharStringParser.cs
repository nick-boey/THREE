...
internal readonly struct Type2Instruction
{
    ...
    public float ReadValueAsFixed1616()
    {
        ...
        var fraction_part = (short)((b2 << 8) | b3) / (float)(1 << 16);
    ...
    [ThreadStatic] private static StringBuilder s_dbugSb;

    public override string ToString()
    {
        var merge_flags = Op >> 6; //upper most 2 bits we use as our extension
        //so operator name is lower 6 bits

        var only_operator = Op & 0b111111;
        var op_name = (OperatorName)only_operator;

        if (_dbug_OnlyOp) return op_name.ToString();

        if (s_dbugSb == null) s_dbugSb = new StringBuilder();
...
internal class Type2GlyphInstructionList
{
    ...
    {
        InnerInsts = new List<Type2Instruction>();
    }

    public int Count => InnerInsts.Count;


    internal List<Type2Instruction> InnerInsts { get; }

    public Type2Instruction RemoveLast()
    {
        var last = InnerInsts.Count - 1;
        var _lastInst = InnerInsts[last];
        InnerInsts.RemoveAt(last);
        return _lastInst;
    }
    ...
    public void AddInt(int intValue)
    {
        ...
        debugCheck();
#endif
        InnerInsts.Add(new Type2Instruction(OperatorName.LoadInt, intValue));
    }
...
#endif
        InnerInsts.Add(new Type2Instruction(OperatorName.LoadFloat, float1616Fmt));
    }
    ...
    public void AddOp(OperatorName opName)
    {
        ...
        debugCheck();
#endif
        InnerInsts.Add(new Type2Instruction(opName));
    }
    ...
    public void AddOp(OperatorName opName, int value)
    {
        ...
        debugCheck();
#endif
        InnerInsts.Add(new Type2Instruction(opName, value));
    }

    internal void ChangeFirstInstToGlyphWidthValue()
    {
        //check the first element must be loadint
        if (InnerInsts.Count == 0) return;

        var firstInst = InnerInsts[0];
        if (!firstInst.IsLoadInt) throw new NotSupportedException();
        //the replace
        InnerInsts[0] = new Type2Instruction(OperatorName.GlyphWidth, firstInst.Value);
    }

#if DEBUG
    private void debugCheck()
    {
        if (dbugMark == 5 && InnerInsts.Count > 50)
        {
        }
    }

    public int dbugInstCount => InnerInsts.Count;
    ...
    {
        InnerInsts.dbugDumpInstructionListToFile(filename);
    }
#endif
}
...
internal class Type2CharStringParser
{
        ...
        while (cont && !reader.IsEnd())
        {
            ...
            switch (b0)
            {
                default: //else 32 -255
                {
                    ...
                    if (_doStemCount) _current_integer_count++;
                }
                    break;
                case 255:
                {
                ...
                case (byte)Type2Operator1.shortint: // 28
                    ...
                    var s_b0 = reader.ReadByte();
                    var s_b1 = reader.ReadByte();
                ...
                case (byte)Type2Operator1.hintmask:
                    AddHintMaskToList(ref reader);
                    StopStemCount();
                    break;
                case (byte)Type2Operator1.cntrmask:
                    AddCounterMaskToList(ref reader);
                    StopStemCount();
                    break;
                //-------------------------------------------------------------------
                ...
                {
#if DEBUG
                    if (!reader.IsEnd()) throw new NotSupportedException();
                ...
                case (byte)Type2Operator1.callsubr:
                {
                    ...
                    if (_currentCff1Font != null)
                    {
                        ...
                        if (!inst.IsLoadInt) throw new NotSupportedException();
                        if (_doStemCount) _current_integer_count--;
                        //subr_no must be adjusted with proper bias value 
                        if (_currentCff1Font._localSubrRawBufferList != null)
                            ParseType2CharStringBuffer(
                                _currentCff1Font._localSubrRawBufferList[inst.Value + _localSubrBias]);
                ...
                case (byte)Type2Operator1.callgsubr:
                {
                    if (_currentCff1Font != null)
                    {
                        ...
                        //load global subr
                        ParseType2CharStringBuffer(
                            _currentCff1Font._globalSubrRawBufferList[inst.Value + _globalSubrBias]);
                    }
                }
                    break;
            }
    ...
    {
#if DEBUG
        if (fontdic == null) throw new NotSupportedException();
#endif
    ...
    public Type2GlyphInstructionList ParseType2CharString(byte[] buffer)
    {
        ...
        if (_currentCff1Font == null) throw new NotSupportedException();
        //
    ...
    private void AddMoveToOp(OperatorName op)
    {
        ...
        if (!_foundSomeStem && !_enterPathConstructionSeq)
        {
            ...
            {
                if (_insts.Count % 2 != 0) _insts.ChangeFirstInstToGlyphWidthValue();
            }
            else
            {
                //vmoveto, hmoveto
    ...
    private void AddHintMaskToList(ref SimpleBinaryReader reader)
    {
...
#if DEBUG
            if (_current_integer_count % 2 != 0) throw new NotSupportedException();
#endif
            if (_doStemCount)
        ...
        var properNumberOfMaskBytes = (_hintStemCount + 7) / 8;

        if (reader.Position + properNumberOfMaskBytes >= reader.BufferLength) throw new NotSupportedException();
