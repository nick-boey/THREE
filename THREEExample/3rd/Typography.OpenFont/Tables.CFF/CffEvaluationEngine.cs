...
{
    private readonly Stack<Type2EvaluationStack> _evalStackPool = new();
    ...
    internal void Run(IGlyphTranslator tx, Type2Instruction[] instructionList, float scale = 1)
    {
        ...
        if (scaleTx.IsContourOpened) scaleTx.CloseContour();
    ...
    private void Run(IGlyphTranslator tx, Type2Instruction[] instructionList, ref double currentX, ref double currentY)
    {
        ...
        for (var i = 0; i < instructionList.Length; ++i)
        {
            ...
            var merge_flags = inst.Op >> 6; //upper 2 bits is our extension flags
            switch (merge_flags)
            {
                case 0: //nothing
                    break;
    ...
    {
        if (_evalStackPool.Count > 0) return _evalStackPool.Pop();
    ...
    private class PxScaleGlyphTx : IGlyphTranslator
    {
        ...
        public bool IsContourOpened { get; private set; }
        ...
        {
            IsContourOpened = false;
            _tx.CloseContour();
        }

        public void Curve3(float x1, float y1, float x2, float y2)
        {
            IsContourOpened = true;
...
internal class Type2EvaluationStack
{
...
#endif
    public IGlyphTranslator GlyphTranslator { get; set; }
    ...
    public void R_MoveTo()
    {
        ...
        //see [NOTE4]
#if DEBUG
        if (_currentIndex != 2) throw new NotSupportedException();
#endif


        GlyphTranslator.CloseContour();
        GlyphTranslator.MoveTo((float)(_currentX += _argStack[0]), (float)(_currentY += _argStack[1]));
    ...
    public void H_MoveTo()
    {
        ...
        GlyphTranslator.MoveTo((float)(_currentX += _argStack[0]), (float)_currentY);
        _currentIndex = 0; //clear stack 
    }
    ...
    public void V_MoveTo()
    {
        ...
        //see [NOTE4] 
#if DEBUG
        if (_currentIndex > 1) throw new NotSupportedException();
#endif
        GlyphTranslator.MoveTo((float)_currentX, (float)(_currentY += _argStack[0]));
        _currentIndex = 0; //clear stack 
    }
...
#if DEBUG
        if (_currentIndex % 2 != 0) throw new NotSupportedException();
#endif
        for (var i = 0; i < _currentIndex;)
        {
            GlyphTranslator.LineTo((float)(_currentX += _argStack[i]), (float)(_currentY += _argStack[i + 1]));
            i += 2;
        }
    ...
    public void H_LineTo()
    {
        ...
        if (_currentIndex % 2 != 0)
        {
            //|- dx1 {dya dxb}*  hlineto (6) |-
            //odd number                
            GlyphTranslator.LineTo((float)(_currentX += _argStack[i]), (float)_currentY);
            i++;
            for (; i < _currentIndex;)
            {
                GlyphTranslator.LineTo((float)_currentX, (float)(_currentY += _argStack[i]));
                GlyphTranslator.LineTo((float)(_currentX += _argStack[i + 1]), (float)_currentY);
                i += 2;
            }
        }
        else
        {
            ...
            for (; i < _currentIndex;)
            {
                //line to
                GlyphTranslator.LineTo((float)(_currentX += _argStack[i]), (float)_currentY);
                GlyphTranslator.LineTo((float)_currentX, (float)(_currentY += _argStack[i + 1]));
                //
                i += 2;
            }
        }
    ...
    public void RR_CurveTo()
    {
        ...
        var i = 0;
#if DEBUG
        if (_currentIndex % 6 != 0) throw new NotSupportedException();
    ...
    public void R_CurveLine()
    {
        ...
        while (count > 0)
        {
            ...
            {
                //last one
                GlyphTranslator.LineTo((float)(curX += _argStack[i]), (float)(curY += _argStack[i + 1]));
                break; //exit while
            }
        }
