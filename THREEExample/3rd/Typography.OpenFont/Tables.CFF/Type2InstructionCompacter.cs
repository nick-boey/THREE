...
internal class Type2InstructionCompacter
{
    ...
    private void CompactStep1OnlyLoadInt(List<Type2Instruction> insts)
    {
        ...
        for (var i = 0; i < j; ++i)
        {
            ...
            if (inst.IsLoadInt)
            {
                ...
                //get compact range
                var c1 = GetCompactRange(inst.Value);
                switch (c1)
                {
                    default: throw new NotSupportedException();
                    case CompactRange.None:
                    {
                        if (collecting_count > 0) FlushWaitingNumbers();
                        _step1List.Add(inst);
    ...
    private void CompactStep2MergeLoadIntWithNextCommand()
    {
        ...
        for (var i = 0; i < j; ++i)
        {
            ...
            {
                //has next cmd           
                var merge_flags = IsLoadIntOrMergeableLoadIntExtension((OperatorName)i0.Op);
                if (merge_flags > 0)
                {
                    var i1 = _step1List[i + 1];
    ...
    public Type2Instruction[] Compact(List<Type2Instruction> insts)
    {
        ...
        //reset
        if (_step1List == null) _step1List = new List<Type2Instruction>();
        if (_step2List == null) _step2List = new List<Type2Instruction>();
        _step1List.Clear();
        _step2List.Clear();
        //
    ...
    {
        if (value > sbyte.MinValue && value < sbyte.MaxValue) return CompactRange.SByte;

        if (value > short.MinValue && value < short.MaxValue) return CompactRange.Short;
    ...
    private void dbugReExpandAndCompare_ForStep1(List<Type2Instruction> step1, List<Type2Instruction> org)
    {
        ...
        if (expand1.Count != org.Count)
        {
            ...
            var min = Math.Min(expand1.Count, org.Count);
            for (var i = 0; i < min; ++i)
            {
                var inst_exp = expand1[i];
                var inst_org = org[i];
    ...
    private void dbugReExpandAndCompare_ForStep2(List<Type2Instruction> step2, List<Type2Instruction> org)
    {
        ...
        //--------------------------------------------
        if (expand2.Count != org.Count) throw new NotSupportedException();
