...
internal class WoffReader
{
    ...
    public PreviewFontInfo ReadPreview(BinaryReader reader)
    {
        ...
        //
        var woffTableDirs = ReadTableDirectories(reader);
        if (woffTableDirs == null) return null;
        ...
        var tableEntryCollection = CreateTableEntryCollection(woffTableDirs);
        ...
        using (var decompressStream = new MemoryStream())
        {
            if (Extract(reader, woffTableDirs, decompressStream))
                using (var reader2 = new ByteOrderSwappingBinaryReader(decompressStream))
                {
                    ...
                    var previewFontInfo = openFontReader.ReadPreviewFontInfo(tableEntryCollection, reader2);
    ...
    internal bool Read(Typeface typeface, BinaryReader reader, RestoreTicket ticket)
    {
        ...
        //
        var woffTableDirs = ReadTableDirectories(reader);
        if (woffTableDirs == null) return false;
        //
        //try read each compressed table
        if (DecompressHandler == null)
        {
    ...
    private static WoffHeader ReadWOFFHeader(BinaryReader reader)
    {
        ...
        if (!(b0 == 0x77 && b1 == 0x4f && b2 == 0x46 && b3 == 0x46)) return null;
    ...
    private WoffTableDirectory[] ReadTableDirectories(BinaryReader reader)
    {
        ...
        var tableDirs = new WoffTableDirectory[tableCount];
    ...
    private bool Extract(BinaryReader reader, WoffTableDirectory[] tables, Stream newDecompressedStream)
    {
        for (var i = 0; i < tables.Length; ++i)
        {
            ...
            var compressedBuffer = reader.ReadBytes((int)table.compLength);
