...
{
    private static readonly TripleEncodingTable s_encTable = TripleEncodingTable.GetEncTable();
    ...
    private static void ReconstructGlyfTable(BinaryReader reader, Woff2TableDirectory woff2TableDir, Glyf glyfTable)
    {
        ...
        var nContourStreamSize = reader.ReadUInt32(); //in bytes
        var nPointsStreamSize = reader.ReadUInt32(); //in bytes
        var flagStreamSize = reader.ReadUInt32(); //in bytes
        var glyphStreamSize = reader.ReadUInt32(); //in bytes
        var compositeStreamSize = reader.ReadUInt32(); //in bytes
        var bboxStreamSize = reader.ReadUInt32(); //in bytes
        var instructionStreamSize = reader.ReadUInt32(); //in bytes


        var expected_nCountStartAt = reader.BaseStream.Position;
        var expected_nPointStartAt = expected_nCountStartAt + nContourStreamSize;
        var expected_FlagStreamStartAt = expected_nPointStartAt + nPointsStreamSize;
        var expected_GlyphStreamStartAt = expected_FlagStreamStartAt + flagStreamSize;
        var expected_CompositeStreamStartAt = expected_GlyphStreamStartAt + glyphStreamSize;

        var expected_BboxStreamStartAt = expected_CompositeStreamStartAt + compositeStreamSize;
        var expected_InstructionStreamStartAt = expected_BboxStreamStartAt + bboxStreamSize;
        var expected_EndAt = expected_InstructionStreamStartAt + instructionStreamSize;
        ...
        //--------
#if DEBUG
        if (reader.BaseStream.Position != expected_nPointStartAt) Debug.WriteLine("ERR!!");
#endif
        //
...
#if DEBUG
        if (reader.BaseStream.Position != expected_FlagStreamStartAt) Debug.WriteLine("ERR!!");
#endif
...
#if DEBUG
        if (reader.BaseStream.Position != expected_GlyphStreamStartAt) Debug.WriteLine("ERR!!");
#endif
        ...
        using (var compositeMS = new MemoryStream())
        {
            ...
            for (ushort i = 0; i < j; ++i)
            {
                ...
                allGlyphs[compositeGlyphIndex].compositeHasInstructions =
                    CompositeHasInstructions(compositeReader, compositeGlyphIndex);
            }
        ...
        {
            var tempGlyph = allGlyphs[i];
            if (tempGlyph.instructionLen > 0) glyphs[i].GlyphInstructions = reader.ReadBytes(tempGlyph.instructionLen);
        }
        ...
        byte[] flagStream, ref int flagStreamIndex)
    {
        ...
        {
            var numPoint = pntPerContours[pntContourIndex++]; //increament pntContourIndex AFTER
            pointCount += numPoint;
        ...
        for (var i = 0; i < numContour; ++i)
        {
            ...
            for (; n <= endContour; ++n)
            {
                var f = flagStream[flagStreamIndex++]; //increment the flagStreamIndex AFTER read

                //int f1 = (f >> 7); // most significant 1 bit -> on/off curve

                var xyFormat = f & 0x7F; // remainging 7 bits x,y format  

                var enc = s_encTable[xyFormat]; //0-128 

                var packedXY =
                    glyphStreamReader.ReadBytes(enc.ByteCount -
                ...
                _glyphPoints[n] =
                    new GlyphPointF(curX += x, curY += y, f >> 7 == 0); // most significant 1 bit -> on/off curve 
            }
        }
        ...
        Glyph emptyGlyph)
    {
        ...
        do
        {
            ...
            if (createdGlyphs[glyphIndex] == null)
            {
                ...
                var missingGlyph = ReadCompositeGlyph(createdGlyphs, reader, glyphIndex, emptyGlyph);
    ...
    private class TripleEncodingTable
    {
        ...
        private readonly List<TripleEncodingRecord> _records = new();
...
#if DEBUG
            if (_records.Count != 128) throw new Exception();
            dbugValidateTable();
#endif
        }
        ...
        {
            if (s_encTable == null) s_encTable = new TripleEncodingTable();
            return s_encTable;
        }
#if DEBUG
        private void dbugValidateTable()
        {
            ...
            {
                var tripleRec = _records[xyFormat];
                if (xyFormat < 84)
                {
                    //0-83 inclusive
                    if (tripleRec.ByteCount - 1 != 1) throw new NotSupportedException();
                }
                else if (xyFormat < 120)
                {
                    //84-119 inclusive
                    if (tripleRec.ByteCount - 1 != 2) throw new NotSupportedException();
                }
                else if (xyFormat < 124)
                {
                    //120-123 inclusive
                    if (tripleRec.ByteCount - 1 != 3) throw new NotSupportedException();
                }
                else if (xyFormat < 128)
                {
                    //124-127 inclusive
                    if (tripleRec.ByteCount - 1 != 4) throw new NotSupportedException();
                }
            }
        ...
        private void AddRecord(byte byteCount, byte xbits, byte ybits, ushort deltaX, ushort deltaY, sbyte xsign,
            sbyte ysign)
        {
...
internal class Woff2Reader
{
    ...
    public PreviewFontInfo ReadPreview(BinaryReader reader)
    {
        ...
        var woff2TablDirs = ReadTableDirectories(reader);
        ...
        var compressedBuffer = reader.ReadBytes((int)_header.totalCompressedSize);
        ...
        using (var decompressedStream = new MemoryStream())
        {
            ...
            {
                var tableEntryCollection = CreateTableEntryCollection(woff2TablDirs);
    ...
    private Woff2Header ReadHeader(BinaryReader reader)
    {
        ...
        if (!(b0 == 0x77 && b1 == 0x4f && b2 == 0x46 && b3 == 0x32)) return null;
        header.flavor = reader.ReadUInt32();
        var flavorName = Utils.TagToString(header.flavor);
    ...
    private Woff2TableDirectory[] ReadTableDirectories(BinaryReader reader)
    {
        ...
        for (var i = 0; i < tableCount; ++i)
        {
            ...
            //interprete flags 
            var knowTable = flags & 0x1F; //5 bits => known table or not  

            table.Name =
                knowTable < 63 ? s_knownTableTags[knowTable] : Utils.TagToString(reader.ReadUInt32()); //other tag 
    ...
    private static TableEntryCollection CreateTableEntryCollection(Woff2TableDirectory[] woffTableDirs)
    {
        ...
        {
            var woffTableDir = woffTableDirs[i];
    ...
    {
        if (s_dbugPassVeriKnownTables) return;
        //--------------
        var uniqueNames = new Dictionary<string, bool>();
        foreach (var name in s_knownTableTags)
...
internal class Woff2Utils
{
    ...
    public static short[] ReadInt16Array(BinaryReader reader, int count)
    {
        ...
        for (var i = 0; i < count; ++i) arr[i] = reader.ReadInt16();
        return arr;
    }
    ...
    public static ushort Read255UInt16(BinaryReader reader)
    {
        ...
        if (code == ONE_MORE_BYTE_CODE1) return (ushort)(reader.ReadByte() + LOWEST_UCODE);

        if (code == ONE_MORE_BYTE_CODE2) return (ushort)(reader.ReadByte() + LOWEST_UCODE * 2);
