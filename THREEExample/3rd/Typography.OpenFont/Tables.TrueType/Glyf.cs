...
internal class Glyf : TableEntry
{
    ...
    internal readonly Glyph _emptyGlyph = new(new GlyphPointF[0], new ushort[0], Bounds.Zero, null, 0);
    ...
    {
        var tableOffset = Header.Offset;
        var locations = _glyphLocations;
        ...
        for (var i = 0; i < glyphCount; i++)
        {
            ...
            if (length > 0)
            {
                ...
                if (contoursCount >= 0)
                {
                    ...
                    Glyphs[i] = ReadSimpleGlyph(reader, contoursCount, bounds, (ushort)i);
                }
                else
                {
        ...
        foreach (var glyphIndex in compositeGlyphs)
        {
            ...
            //}
#endif
            Glyphs[glyphIndex] = ReadCompositeGlyph(Glyphs, reader, tableOffset, glyphIndex);
        }
    }
    ...
    private static SimpleGlyphFlag[] ReadFlags(BinaryReader input, int flagCount)
    {
        ...
        while (i < flagCount)
        {
            ...
            else
            {
                ...
                if (HasFlag(flag, SimpleGlyphFlag.Repeat)) repeatCount = input.ReadByte();
            }
    ...
    private static short[] ReadCoordinates(BinaryReader input, int pointCount, SimpleGlyphFlag[] flags,
        SimpleGlyphFlag isByte, SimpleGlyphFlag signOrSame)
    {
    ...
    private static Glyph ReadSimpleGlyph(BinaryReader reader, int contourCount, Bounds bounds, ushort index)
    {
        ...
        var endPoints = reader.ReadUInt16Array(contourCount);
        //-------------------------------------------------------
    ...
    private Glyph ReadCompositeGlyph(Glyph[] createdGlyphs, BinaryReader reader, uint tableOffset,
