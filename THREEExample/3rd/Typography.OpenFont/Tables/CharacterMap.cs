...
internal class CharMapFormat4 : CharacterMap
{
    ...
    public CharMapFormat4(ushort[] startCode, ushort[] endCode, ushort[] idDelta, ushort[] idRangeOffset,
        ushort[] glyphIdArray)
    {
    ...
    public override ushort GetGlyphIndex(int codepoint)
    {
        ...
        if (i >= _endCode.Length || _startCode[i] > codepoint) return 0;
    ...
    public override void CollectUnicodeChars(List<uint> unicodes)
    {
        for (var i = 0; i < _startCode.Length; ++i)
        {
            ...
            for (var u = start; u <= stop; ++u) unicodes.Add(u);
        }
    }
}
...
internal class CharMapFormat12 : CharacterMap
{
    ...
    public override void CollectUnicodeChars(List<uint> unicodes)
    {
        ...
        {
            var start = _startCharCodes[i];
            var stop = _endCharCodes[i];
            for (var u = start; u <= stop; ++u) unicodes.Add(u);
        }
    }
}
...
internal class CharMapFormat6 : CharacterMap
{
    ...
    {
        var u = _startCode;
        for (uint i = 0; i < _glyphIdArray.Length; ++i) unicodes.Add(u + i);
    }
}
...
internal class CharMapFormat14 : CharacterMap
{
    ...
    public override ushort GetGlyphIndex(int character)
    {
        return 0;
    }
    ...
    public ushort CharacterPairToGlyphIndex(int codepoint, ushort defaultGlyphIndex, int nextCodepoint)
    {
        ...
        if (_variationSelectors.TryGetValue(nextCodepoint, out var sel))
        {
            // If the sequence is a non-default UVS, return the mapped glyph

            if (sel.UVSMappings.TryGetValue(codepoint, out var ret)) return ret;
    ...
    {
        public readonly List<int> DefaultEndCodes = new();
        public readonly List<int> DefaultStartCodes = new();
        public readonly Dictionary<int, ushort> UVSMappings = new();
    }
}
