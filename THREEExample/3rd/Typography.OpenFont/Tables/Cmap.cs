...
internal class Cmap : TableEntry
{
    ...
    private readonly Dictionary<int, ushort> _codepointToGlyphs = new();
    private List<CharMapFormat14> _charMap14List;

    private CharacterMap[] _charMaps;
    ...
    public ushort GetGlyphIndex(int codepoint, int nextCodepoint, out bool skipNextCodepoint)
    {
        ...
        if (!_codepointToGlyphs.TryGetValue(codepoint, out var found))
        {
            for (var i = 0; i < _charMaps.Length; ++i)
            {
                var cmap = _charMaps[i];
                ...
                else if (cmap.PlatformId == 3 && cmap.EncodingId == 1)
                {
                    ...
                    var glyphIndex = cmap.GetGlyphIndex(codepoint); //glyphIndex=> gid
                    if (glyphIndex != 0) found = glyphIndex;
                }
            }
        ...
        if (nextCodepoint > 0 && _charMap14List != null)
            foreach (var cmap14 in _charMap14List)
            {
                var glyphIndex = cmap14.CharacterPairToGlyphIndex(codepoint, found, nextCodepoint);
                if (glyphIndex > 0)
                {
    ...
    private static CharMapFormat4 ReadFormat_4(BinaryReader input)
    {
        ...
        var endCode =
            input.ReadUInt16Array(segCount); //Ending character code for each segment, last = 0xFFFF.            
        ...
        var startCode = input.ReadUInt16Array(segCount); //Starting character code for each segment
        var idDelta = input.ReadUInt16Array(segCount); //Delta for all character codes in segment
        var idRangeOffset = input.ReadUInt16Array(segCount); //Offset in bytes to glyph indexArray, or 0   
        //------------------------------------------------------------------------------------ 
        var remainingLen = tableStartEndAt - input.BaseStream.Position;
        var recordNum2 = (int)(remainingLen / 2);
        var glyphIdArray = input.ReadUInt16Array(recordNum2); //Glyph index array                          
    ...
    private static CharMapFormat6 ReadFormat_6(BinaryReader input)
    {
        ...
        var glyphIdArray = input.ReadUInt16Array(entryCount);
    ...
    {
        for (var i = 0; i < _charMaps.Length; ++i) _charMaps[i].CollectUnicodeChars(unicodes);
    }
}