...
partial class GPOS
{
    private static PosRuleSetTable[] CreateMultiplePosRuleSetTables(long initPos, ushort[] offsets, BinaryReader reader)
    {
        ...
        for (var i = 0; i < j; ++i) results[i] = PosRuleSetTable.CreateFrom(reader, initPos + offsets[i]);
        return results;
    }
    ...
    private static PosLookupRecord[] CreateMultiplePosLookupRecords(BinaryReader reader, int count)
    {
        ...
        for (var n = 0; n < count; ++n) results[n] = PosLookupRecord.CreateFrom(reader);
        return results;
    }
    ...
    private readonly struct PairSet
    {
        public readonly ushort
            secondGlyph; //GlyphID of second glyph in the pair-first glyph is listed in the Coverage table
    ...
    private class ValueRecord
    {
        ...
        public void ReadFrom(BinaryReader reader, ushort valueFormat)
        {
            ...
            if (HasFormat(valueFormat, FMT_XPlacement)) XPlacement = reader.ReadInt16();
            if (HasFormat(valueFormat, FMT_YPlacement)) YPlacement = reader.ReadInt16();
            if (HasFormat(valueFormat, FMT_XAdvance)) XAdvance = reader.ReadInt16();
            if (HasFormat(valueFormat, FMT_YAdvance)) YAdvance = reader.ReadInt16();
            if (HasFormat(valueFormat, FMT_XPlaDevice)) XPlaDevice = reader.ReadUInt16();
            if (HasFormat(valueFormat, FMT_YPlaDevice)) YPlaDevice = reader.ReadUInt16();
            if (HasFormat(valueFormat, FMT_XAdvDevice)) XAdvDevice = reader.ReadUInt16();
            if (HasFormat(valueFormat, FMT_YAdvDevice)) YAdvDevice = reader.ReadUInt16();
        }
    ...
    private class AnchorPoint
    {
        ...
        public override string ToString()
        {
            switch (format)
            {
                ...
                case 3:
                    return format + "(" + xcoord + "," + ycoord + "), xy_device(" + xdeviceTableOffset + "," +
                           ydeviceTableOffset + ")";
            }
        }
#endif
    ...
    private class Mark2ArrayTable
    {
        ...
        public static Mark2ArrayTable CreateFrom(BinaryReader reader, long beginAt, ushort classCount)
        {
            ...
            var offsets = reader.ReadUInt16Array(mark2Count * classCount);
            //read mark2 anchors
    ...
    private class BaseArrayTable
    {
        ...
        public static BaseArrayTable CreateFrom(BinaryReader reader, long beginAt, ushort classCount)
        {
            ...
            var baseAnchorOffsets = reader.ReadUInt16Array(classCount * baseCount);
    ...
    private readonly struct BaseRecord
    {
        //BaseRecord
        //Value 	Type 	Description
    ...
    private class LigatureArrayTable
    {
        ...
        public LigatureAttachTable GetLigatureAttachTable(int index)
        {
            return _ligatures[index];
        }
    }
    ...
    private class LigatureAttachTable
    {
        ...
        public ComponentRecord GetComponentRecord(int index)
        {
            return _records[index];
        }
    }
    ...
    //------ 
    private readonly struct PosLookupRecord
    {
        //PosLookupRecord
        //Value 	Type 	Description
    ...
    private class PosRuleSetTable
    {
        ...
        private void ReadFrom(BinaryReader reader)
        {
            ...
            var posRuleTableOffsets = reader.ReadUInt16Array(posRuleCount);
    ...
    private class PosClassSetTable
    {
        ...
        private void ReadFrom(BinaryReader reader)
        {
            ...
            var posClassRuleOffsets = reader.ReadUInt16Array(posClassRuleCnt);
    ...
    private class PosClassRule
    {
        ...
        public static PosClassRule CreateFrom(BinaryReader reader, long beginAt)
        {
            ...
            var glyphCount = reader.ReadUInt16();
            var posCount = reader.ReadUInt16();
            if (glyphCount > 1) posClassRule.InputGlyphIds = reader.ReadUInt16Array(glyphCount - 1);
