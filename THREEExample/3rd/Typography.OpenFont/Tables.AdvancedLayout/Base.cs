...
public class BASE : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        var horizAxisOffset = reader.ReadUInt16();
        var vertAxisOffset = reader.ReadUInt16();
        uint itemVarStoreOffset = 0;
        if (minorVersion == 1) itemVarStoreOffset = reader.ReadUInt32();
    ...
    private static AxisTable ReadAxisTable(BinaryReader reader)
    {
        ...
        var axisTableStartAt = reader.BaseStream.Position;

        var baseTagListOffset = reader.ReadUInt16();
        var baseScriptListOffset = reader.ReadUInt16();
    ...
    private static string[] ReadBaseTagList(BinaryReader reader)
    {
        ...
        var baseTagCount = reader.ReadUInt16();
        var baselineTags = new string[baseTagCount];
        for (var i = 0; i < baseTagCount; ++i) baselineTags[i] = ConvertToTagString(reader.ReadBytes(4));
        return baselineTags;
    }
    ...
    private static BaseScript[] ReadBaseScriptList(BinaryReader reader)
    {
        ...
        var baseScriptListStartAt = reader.BaseStream.Position;
        var baseScriptCount = reader.ReadUInt16();

        var baseScriptRecord_offsets = new BaseScriptRecord[baseScriptCount];
        for (var i = 0; i < baseScriptCount; ++i)
            ...
            baseScriptRecord_offsets[i] =
                new BaseScriptRecord(ConvertToTagString(reader.ReadBytes(4)), reader.ReadUInt16());
        var baseScripts = new BaseScript[baseScriptCount];
        for (var i = 0; i < baseScriptCount; ++i)
        {
            ...
            //
            var baseScipt = ReadBaseScriptTable(reader);
    ...
    private static BaseScript ReadBaseScriptTable(BinaryReader reader)
    {
        ...
        var baseScriptTableStartAt = reader.BaseStream.Position;
        ...
        var baseValueOffset = reader.ReadUInt16();
        var defaultMinMaxOffset = reader.ReadUInt16();
        var baseLangSysCount = reader.ReadUInt16();
        ...
        if (baseLangSysCount > 0)
        {
            ...
            for (var i = 0; i < baseLangSysCount; ++i)
                ...
                baseLangSysRecords[i] =
                    new BaseLangSysRecord(ConvertToTagString(reader.ReadBytes(4)), reader.ReadUInt16());
        }
    ...
    private static BaseValues ReadBaseValues(BinaryReader reader)
    {
        ...
        var baseValueTableStartAt = reader.BaseStream.Position;

        //
        var defaultBaselineIndex = reader.ReadUInt16();
        var baseCoordCount = reader.ReadUInt16();
        var baseCoords_Offset = reader.ReadUInt16Array(baseCoordCount);

        var baseCoords = new BaseCoord[baseCoordCount];
    ...
    private static BaseCoord ReadBaseCoordTable(BinaryReader reader, long pos)
    {
        ...
        var baseCoordFormat = reader.ReadUInt16();
    ...
    private static MinMax ReadMinMaxTable(BinaryReader reader)
    {
        ...
        var startMinMaxTableAt = reader.BaseStream.Position;
        //
        var minMax = new MinMax();
        var minCoordOffset = reader.ReadUInt16();
        var maxCoordOffset = reader.ReadUInt16();
        var featMinMaxCount = reader.ReadUInt16();
        ...
        //----------
        if (minCoordOffset > 0) minMax.minCoord = ReadBaseCoordTable(reader, startMinMaxTableAt + minCoordOffset);
        if (maxCoordOffset > 0) minMax.maxCoord = ReadBaseCoordTable(reader, startMinMaxTableAt + maxCoordOffset);
        ...
        if (minMaxFeatureOffsets != null)
        {
            ...
            {
                var featureMinMaxOffset = minMaxFeatureOffsets[i];
    ...
    private readonly struct BaseScriptRecord
    {
    ...
    private readonly struct FeatureMinMaxOffset
    {
        public readonly string featureTableTag;
