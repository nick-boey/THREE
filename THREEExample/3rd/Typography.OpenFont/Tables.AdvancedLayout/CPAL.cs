...
public class CPAL : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        var numPaletteEntries = reader.ReadUInt16(); // XXX: unused?
        var numPalettes = reader.ReadUInt16();
        ColorCount = reader.ReadUInt16(); //numColorRecords
        var offsetFirstColorRecord =
            reader.ReadUInt32(); //Offset from the beginning of CPAL table to the first ColorRecord.
        Palettes = reader
            .ReadUInt16Array(
                numPalettes); //colorRecordIndices, Index of each palette’s first color record in the combined color record array.
