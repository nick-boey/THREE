...
internal class MVar : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        var valueRecordSize = reader.ReadUInt16();
        var valueRecordCount = reader.ReadUInt16();
        var itemVariationStoreOffset = reader.ReadUInt16();

        valueRecords = new ValueRecord[valueRecordCount];

        for (var i = 0; i < valueRecordCount; ++i)
        {
            var recStartAt = reader.BaseStream.Position;
            ...
            reader.BaseStream.Position =
                recStartAt +
                valueRecordSize; //**Implementations must use the valueRecordSize field to determine the start of each record.**
        }
    ...
    {
        private static readonly Dictionary<string, ValueTagInfo> s_registerTags = new();
