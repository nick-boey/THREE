...
internal class TupleVariationHeader
{
    ...
    public static TupleVariationHeader Read(BinaryReader reader, int axisCount)
    {
        ...
        var flags = (tupleIndex >> 12) & 0xF; //The high 4 bits are flags(see below).
        header.flags = flags; //The high 4 bits are flags(see below).
        header.indexToSharedTupleRecArray =
            (ushort)(tupleIndex & 0x0FFF); // The low 12 bits are an index into a shared tuple records array.


        if ((flags & ((int)TupleIndexFormat.EMBEDDED_PEAK_TUPLE >> 12)) ==
            (int)TupleIndexFormat.EMBEDDED_PEAK_TUPLE >> 12)
            //TODO:...
            header.peakTuple = TupleRecord.ReadTupleRecord(reader, axisCount);
        if ((flags & ((int)TupleIndexFormat.INTERMEDIATE_REGION >> 12)) ==
            (int)TupleIndexFormat.INTERMEDIATE_REGION >> 12)
        {
            //TODO:...
...
internal readonly struct TupleRecord
{
    ...
    public TupleRecord(float[] coords)
    {
        this.coords = coords;
    }
#if DEBUG
