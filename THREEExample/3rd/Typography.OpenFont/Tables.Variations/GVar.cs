...
internal class GVar : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        var glyphVariationDataArrayOffset = reader.ReadUInt32();
        ...
        if ((flags & 0x1) == 0)
        {
            ...
            for (var i = 0; i < glyphVariationDataOffsets.Length; ++i) glyphVariationDataOffsets[i] *= 2;
        }
        else
        {
            //Offset32
        ...
        var glyphVariableData_startAt = beginAt + glyphVariationDataArrayOffset;
    ...
    private void ReadSharedTupleArray(BinaryReader reader, ushort sharedTupleCount)
    {
        ...
        for (var t = 0; t < sharedTupleCount; ++t) tupleRecords[t] = TupleRecord.ReadTupleRecord(reader, axisCount);
    ...
    private static void ReadPackedPoints(BinaryReader reader, int point_count, List<ushort> packPoints)
    {
        ...
        while (point_read < point_count)
        {
            ...
            var point_run_count = (controlByte & 0x7F) + 1;
    ...
    private GlyphVariableData ReadGlyphVariationData(BinaryReader reader)
    {
        ...
        var tupleCount =
            tupleVariationCount & 0xFFF; //low 12 bits are the number of tuple variation tables for this glyph

        var tupleHaders = new TupleVariationHeader[tupleCount];
        glyphVarData.tupleHeaders = tupleHaders;

        for (var i = 0; i < tupleCount; ++i) tupleHaders[i] = TupleVariationHeader.Read(reader, axisCount);
        ...
        var flags = tupleVariationCount >> 12; //The high 4 bits are flags, 
        ...
        {
            var header = tupleHaders[i];

            var dataSize = header.variableDataSize;
            var expect_endAt = reader.BaseStream.Position + dataSize;
            ...
            if ((header.flags & ((int)TupleIndexFormat.PRIVATE_POINT_NUMBERS >> 12)) ==
                (int)TupleIndexFormat.PRIVATE_POINT_NUMBERS >> 12)
            {
            ...
            {
                var controlByte = reader.ReadByte();
                var number_in_run = (controlByte & 0x3F) + 1;

                var flags01 = (controlByte >> 6) << 6;
...
#if DEBUG
            //ensure!
            if (packedDeltasXY.Count % 2 != 0) Debugger.Break();
            //ensure!
            if (reader.BaseStream.Position != expect_endAt) Debugger.Break();
#endif
        }
