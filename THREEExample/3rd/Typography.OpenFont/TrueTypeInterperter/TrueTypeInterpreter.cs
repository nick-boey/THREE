...
public class TrueTypeInterpreter
{
    ...
    {
        var glyph = _currentTypeFace.GetGlyph(glyphIndex);
        //-------------------------------------------
        ...
        float glyphSizeInPixel)
    {
        ...
        for (var i = orgLen + 3; i >= 0; --i) newGlyphPoints[i].ApplyScale(pxScale);
        ...
        //
        if (UseVerticalHinting) ApplyScaleOnlyOnXAxis(newGlyphPoints, agg_x_scale);
        ...
        //6. scale back
        if (UseVerticalHinting) ApplyScaleOnlyOnXAxis(newGlyphPoints, 1f / agg_x_scale);
    ...
    private static void ApplyScaleOnlyOnXAxis(GlyphPointF[] glyphPoints, float xscale)
    {
        ...
        for (var i = glyphPoints.Length - 1; i >= 0; --i) glyphPoints[i].ApplyScaleOnlyOnXAxis(xscale);
    }
}
...
internal class SharpFontInterpreter
{
    ...
    public SharpFontInterpreter(int maxStack, int maxStorage, int maxFunctions, int maxInstructionDefs,
    ...
    public void SetControlValueTable(int[] cvt, float scale, float ppem, byte[] cvProgram)
    {
        ...
        if (cvProgram != null)
        {
            ...
            else
            {
                ...
                _cvtState = _state;
                _cvtState.Freedom = new Vector2(1, 0);
                _cvtState.Projection = new Vector2(1, 0);
                _cvtState.DualProjection = new Vector2(1, 0);
            ...
            switch (opcode)
            {
                ...
                case OpCode.INSTCTRL:
                {
                    ...
                    if (selector >= 1 && selector <= 2)
                    {
                        ...
                        if (_stack.Pop() == 0)
                            _state.InstructionControl =
                                (InstructionControlFlags)((int)_state.InstructionControl & ~bit);
                        else
                ...
                case OpCode.SHP1:
                {
                    ...
                    break;
                case OpCode.SHPIX: ShiftPoints(_state.Freedom * _stack.PopFloat()); break;
                case OpCode.SHC0:
                case OpCode.SHC1:
                {
                    Zone zone;
                ...
                case OpCode.MSIRP1:
                {
                    ...
                    if (_zp1.IsTwilight)
                    {
                        _zp1.Original[pointIndex].P =
                            _zp0.Original[_state.Rp0].P + _state.Freedom * targetDistance / _fdotp;
    ...
    private static void InterpolatePointsXAxis(GlyphPointF[] current, GlyphPointF[] original, int start, int end,
        int ref1, int ref2)
    {
        if (start > end)
            return;
    ...
    private static void InterpolatePointsYAxis(GlyphPointF[] current, GlyphPointF[] original, int start, int end,
        int ref1, int ref2)
    {
        if (start > end)
            return;
