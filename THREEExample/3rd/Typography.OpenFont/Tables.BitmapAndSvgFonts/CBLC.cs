...
internal class CBLC : TableEntry
{
    ...
    {
        var cblcBeginPos = reader.BaseStream.Position;
        var majorVersion = reader.ReadUInt16(); //3
        var minorVersion = reader.ReadUInt16(); //0
        ...
        for (var i = 0; i < numSizes; ++i) bmpSizeTables[i] = BitmapSizeTable.ReadBitmapSizeTable(reader);
        ...
        for (var n = 0; n < numSizes; ++n)
        {
            ...
            var numberofIndexSubTables = bmpSizeTable.numberOfIndexSubTables;
            ...
            for (uint i = 0; i < numberofIndexSubTables; ++i)
            {
                ...
                reader.BaseStream.Position = cblcBeginPos + bmpSizeTable.indexSubTableArrayOffset +
                                             indexSubTableArr.additionalOffsetToIndexSubtable;

                var result = subTables[i] = IndexSubTableBase.CreateFrom(bmpSizeTable, reader);
    ...
    public Glyph[] BuildGlyphList()
    {
        ...
        var numSizes = _bmpSizeTables.Length;
        for (var n = 0; n < numSizes; ++n)
        {
            var bmpSizeTable = _bmpSizeTables[n];
            var numberofIndexSubTables = bmpSizeTable.numberOfIndexSubTables;
            for (uint i = 0; i < numberofIndexSubTables; ++i) bmpSizeTable.indexSubTables[i].BuildGlyphList(glyphs);
        }
