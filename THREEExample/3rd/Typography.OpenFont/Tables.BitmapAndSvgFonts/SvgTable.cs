...
internal class SvgTable : TableEntry
{
    ...
    protected override void ReadContentFrom(BinaryReader reader)
    {
        ...
        for (var i = 0; i < numEntries; ++i)
        {
            ...
            if (entry.svgDocLength == 0) throw new NotSupportedException();
    ...
    public bool ReadSvgContent(ushort glyphIndex, StringBuilder outputStBuilder)
    {
        if (_dicSvgEntries == null)
        {
            ...
            for (var i = 0; i < _entries.Length; ++i) _dicSvgEntries.Add(_entries[i].startGlyphID, i);
        }

        if (_dicSvgEntries.TryGetValue(glyphIndex, out var entryIndex))
        {
            var docEntry = _entries[entryIndex];

            if (docEntry.compressed) throw new NotSupportedException();
