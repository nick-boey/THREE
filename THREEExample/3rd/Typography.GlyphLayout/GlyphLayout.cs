...
{
    private readonly List<UnscaledGlyphPlan> _list = new();
...
{
    //
    public static GlyphPlanSequence Empty = new();
    ...
    public GlyphPlanSequence(IUnscaledGlyphPlanList glyphPlanList)
    {
        ...
        startAt = 0;
        len = (ushort)glyphPlanList.Count;
        IsRightToLeft = false;
    }
    ...
    public UnscaledGlyphPlan this[int index]
    {
        get
        {
            if (index >= 0 && index < startAt + len) return _glyphPlanList[startAt + index];
    ...
    public float CalculateWidth()
    {
        ...
        //
        var plans = _glyphPlanList;
        var end = startAt + len;
        float width = 0;
        for (var i = startAt; i < end; ++i) width += plans[i].AdvanceX;
        return width;
    }

    public bool IsEmpty()
    {
        return _glyphPlanList == null;
    }
}
...
{
    private readonly Dictionary<int, GlyphLayoutPlanContext> _collection = new();
    ...
    {
        var hash_code = CalculateHash(typeface, scriptLang);
        if (!_collection.TryGetValue(hash_code, out var context))
        {
            var g_sub = typeface.GSUBTable != null
                ? new GlyphSubstitution(typeface, scriptLang.scriptTag, scriptLang.sysLangTag)
                : null;
            var g_pos = typeface.GPOSTable != null
                ? new GlyphSetPosition(typeface, scriptLang.scriptTag, scriptLang.sysLangTag)
                : null;

#if DEBUG
            if (g_sub != null) g_sub.dbugScriptLang = scriptLang.ToString();
            if (g_pos != null) g_pos.dbugScriptLang = scriptLang.ToString();
#endif
    ...
    private static int CalculateHash(Typeface t, ScriptLang scriptLang)
    {
        var hash = 17;
...
internal readonly struct GlyphLayoutPlanContext
{
...
#if DEBUG
internal readonly struct dbugCodePointFromUserChar
{
    /// <summary>
    ///     input codepoint
    /// </summary>
...
public class GlyphLayout
{
    ...
    private static readonly ScriptLang s_latin = new("latn");
    private static readonly ScriptLang s_math = new("math");
#if DEBUG
    private readonly List<dbugCodePointFromUserChar> _dbugReusableCodePointFromUserCharList = new();
#endif
    private readonly GlyphLayoutPlanCollection _layoutPlanCollection = new();


    //not thread-safe*** 
    private readonly List<int> _reusableUserCodePoints = new();

    private GlyphNotFoundHandler _glyphNotFoundHandler;
    public GlyphPosStream _glyphPositions = new();
    private GlyphSetPosition _gpos;
    private GlyphSubstitution _gsub;

    public GlyphIndexList _inputGlyphs = new(); //reusable input glyph
    ...
    //unscaled version
    public IGlyphPositions ResultUnscaledGlyphPositions => _glyphPositions;
    ...
    public void Layout(IList<int> inputCodePoints, int startAt, int len)
    {
        ...
        for (var i = 0; i < end; ++i)
        {
            ...
            var glyphIndex = _typeface.GetGlyphIndex(cur_codepoint, next_codepoint, out var skipNextCodepoint);
    ...
    {
        if (_needPlanUpdate) UpdateLayoutPlan();
        ...
        //so count again.
        var finalGlyphCount = glyphs.Count;
        //----------------------------------------------  
        ...
        for (var i = 0; i < finalGlyphCount; ++i)
        {
            ...
            glyphs.GetGlyphIndexAndMap(i,
                out var glyphIndex,
                out var input_codepointOffset,
                out var input_mapLen);
            //
            var orgGlyph = _typeface.GetGlyph(glyphIndex);
            //this is original value WITHOUT fit-to-grid adjust
            _glyphPositions.AddGlyph(input_codepointOffset, glyphIndex, orgGlyph);
        }

        var posTech = PositionTechnique;
        ...
        if (_scriptLang.scriptTag == s_math.scriptTag) //***
            ...
            {
                var pos_count = _glyphPositions.Count;
                ...
                //(0.33f/ scale_to_px(8pt)) > original_correction
                var none_sig_correction =
                    0.33f / _typeface.CalculateScaleToPixelFromPointSize(8); //assume at 8 pt size font

                short prevGlyph_italic_correction = 0;

                for (var i = 0; i < pos_count; ++i)
                {
                    var glyph = _typeface.GetGlyph(_glyphPositions[i].glyphIndex);

                    if (glyph?.MathGlyphInfo?.ItalicCorrection is MathValueRecord value &&
    ...
    {
        var context = _layoutPlanCollection.GetPlanOrCreate(_typeface, _scriptLang);
...
/// </summary>
public class GlyphPosStream : IGlyphPositions
{
    public List<GlyphPos> _glyphPosList = new();
    ...
    {
        var existing = _glyphPosList[index];
    ...
    public void AddGlyph(ushort o_offset, ushort glyphIndex, Glyph glyph)
    {
        if (!Glyph.HasOriginalAdvancedWidth(glyph))
            ...
            Glyph.SetOriginalAdvancedWidth(glyph, Typeface.GetAdvanceWidthFromGlyphIndex(glyphIndex));
...
public struct GlyphPos
{
    ...
    public override string ToString()
    {
        return glyphIndex + "(" + xoffset + "," + yoffset + ")";
    }
#endif
}