...
{
    private static readonly Dictionary<object, FilePicker> _filePickers = new();
    ...
    private List<string> GetFileSystemEntries(string fullName)
    {
        ...
        foreach (var f in Directory.GetFiles(fullName, "*", SearchOption.TopDirectoryOnly))
            if (!OnlyAllowFolders)
            {
                if (AllowedExtensions == null || AllowedExtensions.Contains("*.*")) files.Add(f);
                if (AllowedExtensions != null)
                {
...
public class AssimpLoaderExample : GradientBackgroundShaderExample
{
    ...
    public override void Init()
    {
        ...
        var picker = FilePicker.GetFilePicker(this, currentDir, "*.*");
        AddGuiControlsAction = () =>
        {
            if (ImGui.Button("File Open")) ImGui.OpenPopup("select file");
            var result = false;
