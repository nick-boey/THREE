...
public class CinematicCameraExample : Example
{
    ...
    private readonly float radius = 100;
    private Object3D INTERSECTED;
    private Raycaster raycaster;
    private float theta;

    public CinematicCameraExample()
    {
    ...
    public override void Init()
    {
        ...
        (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalLength"] as GLUniform)
            ["value"] = 15.0f;
        (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalDepth"] as GLUniform)
            ["value"] = 3.0f;
    ...
    {
        var focalLength =
            (float)(((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalLength"] as
                GLUniform)["value"];
        if (ImGui.SliderFloat("focalLength", ref focalLength, 1.0f, 135f))
            (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalLength"] as GLUniform)
                ["value"] = focalLength;
        var fstop =
            (float)(((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["fstop"] as GLUniform)
            ["value"];
        if (ImGui.SliderFloat("focalstop", ref fstop, 1.8f, 22f))
            (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["fstop"] as GLUniform)
                ["value"] = fstop;
        var focalDepth =
            (float)(((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalDepth"] as
                GLUniform)["value"];
        if (ImGui.SliderFloat("focalDepth", ref focalDepth, 1.8f, 22f))
            (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["focalDepth"] as GLUniform)
                ["value"] = focalDepth;
        var showFocus =
            (int)(((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["showFocus"] as
                GLUniform)["value"];
        var showFocusFlag = showFocus >= 1 ? true : false;
        if (ImGui.Checkbox("showFocus", ref showFocusFlag))
            (((camera as CinematicCamera).postprocessing["bokeh_uniforms"] as GLUniforms)["showFocus"] as GLUniform)
                ["value"] = showFocusFlag ? 1 : 0;
    }
}