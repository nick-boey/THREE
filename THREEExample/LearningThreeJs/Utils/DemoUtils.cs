...
public class DemoUtils
{
    ...
    private static void CreateBoundingWall(Scene scene)
    {
        ...
        var wallMaterial = new MeshPhongMaterial { Color = Color.Hex(0xa0522d) };
    ...
    internal static void InitDefaultDirectionalLighting(Scene scene, Vector3 initialPosition = null)
    {
        ...
        var ambientLight = new AmbientLight(Color.Hex(0x343434));
    ...
    private static void CreateGroundPlane(Scene scene)
    {
        ...
        var planeMaterial = new MeshPhongMaterial { Color = Color.Hex(0x9acd32) };
    ...
    private static void CreateHouse(Scene scene)
    {
        ...
        // create the mesh
        var roofMesh = new Mesh(roof, new MeshPhongMaterial { Color = Color.Hex(0x8b7213) });
        var baseMesh = new Mesh(baseGeom, new MeshPhongMaterial { Color = Color.Hex(0xffe4c4) });
    ...
    {
        var trunk = new BoxGeometry(1, 8);
        var leaves = new SphereGeometry(4);

        var trunkMesh = new Mesh(trunk, new MeshPhongMaterial { Color = Color.Hex(0x8b4513) });

        var leavesMesh = new Mesh(leaves, new MeshPhongMaterial { Color = Color.Hex(0x00ff00) });
    ...
    public static Mesh AddGroundPlane(Scene scene)
    {
        ...
        var planeMaterial = new MeshPhongMaterial { Color = Color.Hex(0xffffff) };
        var plane = new Mesh(planeGeometry, planeMaterial);
        plane.ReceiveShadow = true;
    ...
    public static void SetMaterialGroup(Material material, Object3D group)
    {
        ...
        else if (group is Group) group.Children.ForEach(child => { SetMaterialGroup(material, child); });
    }
    ...
    public static Mesh CreateMultiMaterialObject(Geometry geometry, List<Material> materials)
    {
        var mesh = new Mesh();

        for (var i = 0; i < materials.Count; i++) mesh.Add(new Mesh(geometry, materials[i]));
        return mesh;
    }
    ...
    public static Mesh AppliyMeshStandardMaterial(Geometry geometry, ref Material material)
    {
        ...
        {
            material = new MeshStandardMaterial { Color = Color.Hex(0xff0000) };
            material.Side = Constants.DoubleSide;
        }
    ...
    public static void SetRandomColors(Object3D object3D)
    {
        ...
        if (children.Count > 0)
        {
            foreach (var e in children) SetRandomColors(e);
        }
        else
        {
            if (object3D is Mesh)
            {
                ...
                {
                    object3D.Material.Color = new Color().Random();
                    if (object3D.Material.Name.IndexOf("building") == 0)
                    {
                        object3D.Material.Emissive = new Color(0x444444);
