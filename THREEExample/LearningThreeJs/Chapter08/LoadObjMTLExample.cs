...
public class LoadObjMTLExample : Example
{
    ...
    private void buildScene()
    {
        ...
        var wingsposition = new List<int> { 0, 2, 4, 6 };

        wingsposition.ForEach(i => { mesh.Children[i].Rotation.Z = 0.3f * (float)Math.PI; });

        var wingsposition1 = new List<int> { 1, 3, 5, 7 };

        wingsposition1.ForEach(i => { mesh.Children[i].Rotation.Z = -0.3f * (float)Math.PI; });
