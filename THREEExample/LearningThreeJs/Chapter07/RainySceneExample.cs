...
public class RainySceneExample : Example
{
    ...
    private void CreatePointCloud(float size, bool transparent, float opacity, bool sizeAttenuation, Color color)
    {
        ...
        {
            Size = size, Opacity = opacity, Transparent = transparent, SizeAttenuation = sizeAttenuation, Color = color,
            Map = texture
        };
        ...
        {
            var particle = new Particle((float)random.NextDouble() * range, (float)random.NextDouble() * range * 1.5f,
                1 + i / 100.0f);
