...
{
    private Color ControlColor = Color.Hex(0x00ff00);

    public CombinedMaterialExample()
    {
    ...
    public override void Init()
    {
        ...
        AddGuiControlsAction = () => { AddBasicMaterialSettings(null, ""); };
    }
    ...
    public override void AddCube()
    {
        ...
        var cube = SceneUtils.CreateMultiMaterialObject(cubeGeometry,
            new List<Material> { colorMaterial, cubeMaterial });
    ...
    public override void AddBasicMaterialSettings(Material material, string name)
    {
        ...
        var color = new Vector3(ControlColor.R, ControlColor.G, ControlColor.B);
        if (ImGui.ColorPicker3("color", ref color)) ControlColor = new Color(color.X, color.Y, color.Z);


        ImGui.SliderFloat("rotationSpeed", ref rotationSpeed, 0, 20);
        if (ImGui.Button("addCube")) AddCube();
        if (ImGui.Button("removeCube")) RemoveCube();
        if (ImGui.SliderFloat("cameraNear", ref camera.Near, 0, 100))
            camera.UpdateProjectionMatrix();

        if (ImGui.SliderFloat("cameraFar", ref camera.Far, 50, 200)) camera.UpdateProjectionMatrix();
    }
}