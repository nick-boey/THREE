...
namespace THREEExample.Learning.Chapter11
{
    public class EffectComposerTemplate : Example
    {
        ...
        float unrealStrength = 1.0f;
        int unrealResolution = 256;
        float unrealRadius = 0.1f;
        ...
        public virtual void AddUnrealBloomPassControl(string rootName, BloomPass bloom)
        {
            ...
            if (ImGui.TreeNode(rootName))
            {
                if (ImGui.SliderInt("resolution", ref unrealResolution, 2, 1024))
                {
                    changed = true;
                }
                if (ImGui.SliderFloat("strength", ref unrealStrength, 0, 10))
                {
                    changed = true;
                }
                if (ImGui.SliderFloat("radius", ref unrealRadius, 0f, 10.0f))
                {
                    changed = true;
                }
                if (ImGui.SliderFloat("threshold", ref threshold, 0, 0.2f))
                {
                    changed = true;
                }
                if (changed)
                {
                    UnrealBloomPass pass = new UnrealBloomPass(new Vector2(unrealResolution,unrealResolution),unrealStrength, unrealRadius,threshold);
                    bloomComposer.Passes[1] = pass;
                }
                ImGui.TreePop();
            }
