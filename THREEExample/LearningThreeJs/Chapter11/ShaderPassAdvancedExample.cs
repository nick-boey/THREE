...
{
    private ShaderPass bleachFilter;

    private float bleachOpacity = 1.0f;
    private ShaderPass focusShader;
    private ShaderPass FXAAShader;
    ...
    public override void Init()
    {
        ...
        cube.Rotation.X = (float)Math.PI / 2;
    ...
    private void AddFXAAShaderControl(ShaderPass pass)
    {
        if (ImGui.TreeNode("FXAAShader"))
        {
            if (ImGui.Checkbox("enabled", ref pass.Enabled))
                (pass.uniforms["resolution"] as GLUniform)["value"] =
                    new Vector2(1.0f / glControl.Width, 1.0f / glControl.Height);
            ImGui.TreePop();
        }
    }
