...
{
    private ShaderPass focusShader;
    private Mesh groundPlane;

    private float h = 1.0f / 512.0f;
    private float hh = 1.0f / 512.0f;
    private ShaderPass horBlurShader;
    private ShaderPass horTiltShiftShader;
    private float hr = 0.35f;
    private float sampleDistance = 0.94f;
    private ShaderPass triangleBlurShader;
    private float v = 1.0f / 512.0f;
    private ShaderPass verBlurShader;
    private ShaderPass verTiltShiftShader;
    private float vr = 0.35f;
    private float vv = 1.0f / 512.0f;
    private float waveFactor = 0.00125f;
    ...
    public override void Init()
    {
        ...
        var nBoxes = 51;
        var mBoxes = 51;
        var colors = new[] { 0x66ff00, 0x6600ff, 0x0066ff, 0xff6600, 0xff0066 };
    ...
    private void AddHorizontalBlur(ShaderPass pass)
    {
        if (ImGui.TreeNode("horizontalBlur"))
        {
            ...
            if (ImGui.SliderFloat("h", ref h, 0.0f, 0.01f)) (pass.uniforms["h"] as GLUniform)["value"] = h;
    ...
    private void AddVerticalBlur(ShaderPass pass)
    {
        ...
        {
            ImGui.Checkbox("enabled", ref pass.Enabled);
            if (ImGui.SliderFloat("v", ref v, 0.0f, 0.01f)) (pass.uniforms["v"] as GLUniform)["value"] = v;
    ...
    private void AddHorizontalTiltShift(ShaderPass pass)
    {
        ...
        {
            ImGui.Checkbox("enabled", ref pass.Enabled);
            if (ImGui.SliderFloat("r", ref hr, 0.0f, 1.0f)) (pass.uniforms["r"] as GLUniform)["value"] = hr;
            if (ImGui.SliderFloat("h", ref hh, 0.0f, 0.01f)) (pass.uniforms["h"] as GLUniform)["value"] = hh;
    ...
    private void AddVerticalTiltShift(ShaderPass pass)
    {
        ...
        {
            ImGui.Checkbox("enabled", ref pass.Enabled);
            if (ImGui.SliderFloat("r", ref vr, 0.0f, 1.0f)) (pass.uniforms["r"] as GLUniform)["value"] = vr;
            if (ImGui.SliderFloat("v", ref vv, 0.0f, 0.01f)) (pass.uniforms["v"] as GLUniform)["value"] = vv;
