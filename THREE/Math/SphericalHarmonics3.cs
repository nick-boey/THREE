...
{
    public List<Vector3> Coefficients = new();
    ...
    {
        for (var i = 0; i < 9; i++) Coefficients[i] += sh.Coefficients[i];
        return this;
    }

    public SphericalHarmonics3 Scale(float s)
    {
        for (var i = 0; i < 9; i++) Coefficients[i] *= s;
        return this;
    }

    public SphericalHarmonics3 Lerp(SphericalHarmonics3 sh, float alpha)
    {
        for (var i = 0; i < 9; i++) Coefficients[i].Lerp(sh.Coefficients[i], alpha);
        return this;
    }
