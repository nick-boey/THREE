...
public struct Color
{
    ...
    {
        hex = (int)Math.Floor((decimal)hex);
    ...
    {
        return c < 0.04045 ? c * 0.0773993808f : (float)Math.Pow(c * 0.9478672986 + 0.0521327014, 2.4);
    }

    public float LinearToSRGB(float c)
    {
        return c < 0.0031308f ? c * 12.92f : 1.055f * (float)Math.Pow(c, 0.41666) - 0.055f;
    }
    ...
    public Color Set(object value)
    {
        if (value != null && value is Color)
            this = (Color)value;
        else if (value is int) SetHex((int)value);
    ...
    public float[] ToArray(float[] array, int? offset)
    {
        ...
        array[(int)offset] = R;
        array[(int)offset + 1] = G;
        array[(int)offset + 2] = B;
