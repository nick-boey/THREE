...
public class STLLoader
{
    public static BufferGeometry ParseBinary(string filePath)
    {
        ...
        using (var reader = new BinaryReader(File.Open(filePath, FileMode.Open)))
        {
            ...
            for (var index = 0; index < 80 - 10; index++)
                if (reader.ReadChar().Equals('C') && reader.ReadChar().Equals('O') && reader.ReadChar().Equals('L') &&
                    reader.ReadChar().Equals('O'))
            ...
            var vertices = new List<float>(new float[faces * 3 * 3]);
            var normals = new List<float>(new float[faces * 3 * 3]);
            var colors = new List<float>(new float[faces * 3 * 3]);
    ...
    public static BufferGeometry ParseASCII(string filePath)
    {
        ...
        var patternSolid = new Regex(@"^solid([\s\S]*?)endsolid$");
        var patternFace = new Regex(@"facet([\s\S]*?)endfacet");
        var patternVertex = new Regex("vertex" + patternFloat + patternFloat + patternFloat);
        var patternNormal = new Regex("normal" + patternFloat + patternFloat + patternFloat);
        ...
        var solidCollection = patternSolid.Matches(alltext);
        foreach (Match s in solidCollection)
        {
            startVertex = endVertex;
            var solid = s.Groups[1].Value;
            var faceCollection = patternFace.Matches(solid);
            ...
            foreach (Match n in faceCollection)
            {
                ...
                var normalCollection = patternNormal.Matches(text);
                ...
                var vertexCollection = patternVertex.Matches(text);
                foreach (Match v in vertexCollection)
                {
                    vertices.Add(float.Parse(v.Groups[1].Value, CultureInfo.InvariantCulture.NumberFormat),
                        float.Parse(v.Groups[2].Value, CultureInfo.InvariantCulture.NumberFormat),
                        float.Parse(v.Groups[3].Value, CultureInfo.InvariantCulture.NumberFormat));
                ...
                if (normalCounterFace != 1)
                    Trace.TraceError("THREE.STLLoader:Something isn't right with the normal of face nubmer {0}",
                        faceCounter);
                if (vertexCountPerFace != 3)
                    Trace.TraceError("THREE.STLLoader:Something isn't right with the verticies of face number {0}",
                        faceCounter);
