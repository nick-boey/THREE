...
public class ShapeUtils
{
    ...
    public static List<List<int>> TriangulateShape(List<Vector3> contour, List<List<Vector3>> holes)
    {
        ...
        List<Vector2> contour2 = new();
        List<List<Vector2>> holes2 = new();


        RemoveDupEndPts(contour);
        AddContour(vertices, contour);

        holes.ForEach(delegate(List<Vector3> e) { RemoveDupEndPts(e); });
        ...
        for (var i = 0; i < triangles.Count; i += 3) faces.Add(triangles.GetRange(i, 3));
    ...
    public static void RemoveDupEndPts(List<Vector3> points)
    {
        ...
        if (l > 2 && points[l - 1].Equals(points[0])) points.RemoveAt(points.Count - 1);
    }
