...
public class ShapePath
{
    ...
    private List<Shape> ToShapesNoHoles(List<Path> inSubpaths)
    {
        ...
        for (var i = 0; i < inSubpaths.Count; i++)
        {
    ...
    public List<Shape> ToShapes(bool isCCW, bool noHoles)
    {
        ...
        for (var i = 0; i < SubPaths.Count; i++)
        {
            ...
            if (solid)
            {
                ...
                (newShapes[mainIdx]["s"] as Shape).Curves = tmpPath.Curves;
        ...
        if (newShapes.Count > 1)
        {
            ...
            for (int sIdx = 0, sLen = newShapes.Count; sIdx < sLen; sIdx++)
            {
                ...
                {
                    var ho = (Hashtable)sho[hIdx];
                    var hole_unassigned = true;


                    for (var s2Idx = 0; s2Idx < newShapes.Count; s2Idx++)
                        if (IsPointInsidePolygon(ho["p"] as Vector3, (List<Vector3>)newShapes[s2Idx]["p"]))
                        {
                            if (sIdx != s2Idx)
                                toChange.Add(new Hashtable { { "froms", sIdx }, { "tos", s2Idx }, { "hole", hIdx } });
                            if (hole_unassigned)
                            {
                    ...
                    if (hole_unassigned) betterShapeHoles.Add(new Hashtable { { sIdx, ho } });
                }
            }
        ...
        {
            tmpShape = newShapes[i]["s"] as Shape;
            shapes.Add(tmpShape);
            var tmpHoles = (Hashtable)newShapeHoles[i];
