...
public class Curve : Vector3
{
    ...
    {
        ArcLengthDivisions = other.ArcLengthDivisions;
    }
    ...
    public virtual List<Vector3> GetPoints(float? divisions = null)
    {
        ...
        for (var d = 0; d <= divisions; d++) points.Add(GetPoint(d / divisions.Value));
    ...
    public virtual List<Vector3> GetSpacedPoints(float? divisions = null)
    {
        ...
        for (var d = 0; d <= divisions; d++) points.Add(GetPointAt(d / divisions.Value));
    ...
    {
        if (divisions == null) divisions = ArcLengthDivisions;

        if (cacheArcLengths != null &&
            cacheArcLengths.Count == divisions + 1 &&
            !NeedsUpdate)
        ...
        {
            current = GetPoint(p / divisions.Value);
    ...
    public float GetUtoTmapping(float u, float? distance = null)
    {
        ...
        if (arcLengths[i] == targetArcLength) return i / (float)(il - 1);
    ...
    public Hashtable ComputeFrenetFrames(int segments, bool closed)
    {
        ...
        for (var i = 1; i <= segments; i++)
        {
            ...
            if (vec.Length() > float.Epsilon)
            {
                ...
                var theta = (float)Math.Acos(tangents[i - 1].Dot(tangents[i]).Clamp(-1, 1));
        ...
        if (closed)
        {
            var theta = (float)Math.Acos(normals[0].Dot(normals[segments]).Clamp(-1, 1));
