...
public class Font
{
    ...
    public List<Shape> GenerateShapes(string text, float? size)
    {
        ...
        var paths = CreatePaths(text, size.Value, Data);

        for (int p = 0, pl = paths.Count; p < pl; p++) shapes = shapes.Concat(paths[p].ToShapes(false, false)).ToList();
    ...
    public List<ShapePath> CreatePaths(string text, float size, string data)
    {
        ...
        var line_height =
            (yMax - yMin + underlineThickness) *
            scale.Value; // (data.boundingBox.yMax - data.boundingBox.yMin + data.underlineThickness) * scale;
    ...
    public Hashtable CreatePath(string ch, float scale, float offsetX, float offsetY, JObject array)
    {
        ...
        if (glyph == null)
        {
            Console.WriteLine("THREE.Font: character " + ch + " does not exists in font family " + array["familyName"] +
                              ".");
        ...
        if (glyph["o"] != null)
        {
            var outline = glyph["_cachedOutline"] != null
                ? glyph["_cachedOutline"].ToString().Split(' ')
                : glyph["o"].ToString().Split(' ');
            ...
            for (int i = 0, l = outline.Length; i < l;)
            {
                ...
                switch (action)
                {
                    case "m": // moveTo
                        ...
                        x = Convert.ToSingle(outline[i++]) * scale + offsetX;
                        y = Convert.ToSingle(outline[i++]) * scale + offsetY;
                    ...
                    case "q": // quadraticCurveTo
                        ...
                        cpx = Convert.ToSingle(outline[i++]) * scale + offsetX;
                        cpy = Convert.ToSingle(outline[i++]) * scale + offsetY;
                        cpx1 = Convert.ToSingle(outline[i++]) * scale + offsetX;
                        cpy1 = Convert.ToSingle(outline[i++]) * scale + offsetY;
                    ...
                    case "b": // bezierCurveTo
                        ...
                        cpx1 = Convert.ToSingle(outline[i++]) * scale + offsetX;
                        cpy1 = Convert.ToSingle(outline[i++]) * scale + offsetY;
                        cpx2 = Convert.ToSingle(outline[i++]) * scale + offsetX;
                        cpy2 = Convert.ToSingle(outline[i++]) * scale + offsetY;
