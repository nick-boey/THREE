...
{
    public static BufferGeometry ToIndexedBufferGeometry(BufferGeometry src, bool? fullIndex = null,
        int? precision = null)
    {
        precision = precision != null ? precision : 6;
        var prec = (float)Math.Pow(10, precision.Value);
        var precHalf = (float)Math.Pow(10, Math.Floor((double)(precision.Value / 2)));
        var geometry = new BufferGeometry();
        var Array = (src.Attributes["position"] as BufferAttribute<float>).Array;
        var list = new List<float>();
        Dictionary<string, int> vertices = new();
        ...
        int Store(int index, int n)
        {
            var id = "";
            var itemSize = 3;
            var offset = itemSize * index * 3 + n * itemSize;
        ...
        {
            var id = ((int)Math.Floor(x * prec)).ToString() + '_' + ((int)Math.Floor(y * prec)) + '_' +
                     ((int)Math.Floor(z * prec));
        ...
        BufferGeometry IndexBufferGeometry(BufferGeometry src, bool fullIndex)
        {
            ...
            var indexArray = new int[faceCount * 3];
            if (fullIndex)
                ...
                for (int i = 0, l = faceCount; i < l; i++)
                {
                    ...
                    indexArray[i * 3 + 1] = StoreFast(position[offset + 3], position[offset + 4], position[offset + 5],
                        i * 3 + 1);
                    indexArray[i * 3 + 2] = StoreFast(position[offset + 6], position[offset + 7], position[offset + 8],
                        i * 3 + 2);
                }
    ...
    public static BufferGeometry MergeVertices(BufferGeometry geometry, double tolerance = 1e-4)
    {
        ...
        Dictionary<string, int> hashToIndex = new();
        ...
        for (var i = 0; i < attributeNames.Length; i++)
        {
            ...
            if (attr is BufferAttribute<float>)
                tmpAttributes[name] = new BufferAttribute<float>(new float[attr.ItemSize * attr.count], attr.ItemSize,
                    attr.Normalized);
            else if (attr is BufferAttribute<int>)
                tmpAttributes[name] =
                    new BufferAttribute<int>(new int[attr.ItemSize * attr.count], attr.ItemSize, attr.Normalized);
            else
                tmpAttributes[name] = new BufferAttribute<byte>(new byte[attr.ItemSize * attr.count], attr.ItemSize,
                    attr.Normalized);
            ...
            if (morphAttributes != null)
                foreach (var morphAttr in morphAttributes as List<IBufferAttribute>)
                {
                    ...
                    if (morphAttr.Type == typeof(float))
                    {
                        ...
                        (tmpMorphAttributes[name] as List<IBufferAttribute>).Add(
                            new BufferAttribute<float>(array, morphAttr.ItemSize, morphAttr.Normalized));
                    }
                    else if (morphAttr.Type == typeof(int))
                    {
                        var array = new int[morphAttr.count * morphAttr.ItemSize];
                        (tmpMorphAttributes[name] as List<IBufferAttribute>).Add(
                            new BufferAttribute<int>(array, morphAttr.ItemSize, morphAttr.Normalized));
                    }
                    else
                    {
                        var array = new byte[morphAttr.count * morphAttr.ItemSize];
                        (tmpMorphAttributes[name] as List<IBufferAttribute>).Add(
                            new BufferAttribute<byte>(array, morphAttr.ItemSize, morphAttr.Normalized));
                    }
                }
        }
                ...
                for (var j = 0; j < attributeNames.Length; j++)
                {
                    ...
                    for (var k = 0; k < itemSize; k++)
                    {
                        ...
                        if (morphAttributes != null && newMorphArrays != null)
                            for (var m = 0; m < morphAttributes.Count; m++)
                                (newMorphArrays as List<IBufferAttribute>)[m]
                                    .Setter(k, nextIndex, morphAttributes[m].Getter(k, index));
                    }
                }
