...
public class EdgesGeometry : BufferGeometry
{
    ...
    public EdgesGeometry(Geometry geometry, float? thresholdAngle = null)
    {
        ...
        var thresholdDot = (float)Math.Cos(MathUtils.DEG2RAD * thresholdAngle.Value);
        ...
        for (var i = 0; i < indexCount; i += 3)
        {
            ...
            hashes[0] = string.Format("{0},{1},{2}", (int)Math.Round(a.X * precision), (int)Math.Round(a.Y * precision),
                (int)Math.Round(a.Z * precision));
            hashes[1] = string.Format("{0},{1},{2}", (int)Math.Round(b.X * precision), (int)Math.Round(b.Y * precision),
                (int)Math.Round(b.Z * precision));
            hashes[2] = string.Format("{0},{1},{2}", (int)Math.Round(c.X * precision), (int)Math.Round(c.Y * precision),
                (int)Math.Round(c.Z * precision));
