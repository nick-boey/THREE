...
public interface UVGenerator
{
    ...
    List<Vector2> GenerateSideWallUV(Geometry geometry, List<float> vertices, int indexA, int indexB, int indexC,
        int indexD);
}
...
public class WorldUVGenerator : UVGenerator
{
    ...
    public List<Vector2> GenerateSideWallUV(Geometry geometry, List<float> vertices, int indexA, int indexB, int indexC,
        int indexD)
    {
        var a_x = vertices[indexA * 3];
        var a_y = vertices[indexA * 3 + 1];
...
public class ExtrudeBufferGeometry : BufferGeometry
{
    ...
    private List<Vector3> contour = new();
    ...
    private List<float> placeholder = new();

    private int? steps;

    private List<float> uvArray = new();

    private UVGenerator uvgen;

    private List<float> verticesArray = new();

    private int vlen, flen;
    ...
    public void Init(List<Shape> shapes, Hashtable options)
    {
        ...
        SetAttribute("position", new BufferAttribute<float>(verticesArray.ToArray(), 3));

        SetAttribute("uv", new BufferAttribute<float>(uvArray.ToArray(), 2));
    ...
    private void AddShape(Shape shape)
    {
        ...
        var shapePoints = shape.ExtractPoints(curveSegments.Value);

        var vertices = (List<Vector3>)shapePoints["shape"];
        holes = (List<List<Vector3>>)shapePoints["holes"];
        ...
        contour.Clear();
        contour.Concat(vertices); // vertices has all points but contour has only points of circumference
        ...
        List<Vector2> contourMovements = new();
        ...
        for (var b = 0; b < bevelSegments; b++)
        {
            ...
            z = bevelThickness.Value * (float)Math.Cos(t * Math.PI / 2);
            bs = bevelSize.Value * (float)Math.Sin(t * Math.PI / 2) + bevelOffset.Value;
        ...
        {
            var vert = bevelEnabled.Value ? ScalePt2(vertices[i], verticesMovements[i], bs) : vertices[i].ToVector2();
        ...
        for (var b = bevelSegments.Value - 1; b >= 0; b--)
        {
    ...
    private void BuildLidFaces()
    {
        ...
        AddGroup(start, verticesArray.Count / 3 - start);
    }
    ...
    private void BuildSideFaces()
    {
        ...
        AddGroup(start, verticesArray.Count / 3 - start, 1);
    }
    ...
    private Vector2 GetBevelVec(Vector3 inPt, Vector3 inPrev, Vector3 inNext)
    {
        ...
        if (Math.Abs(collinear0) > float.Epsilon)
        {
            // not collinear
            ...
            shrink_by = (float)Math.Sqrt(v_trans_lensq / 2);
        }
        else
        {
            ...
            {
                if (v_next_x > float.Epsilon) direction_eq = true;
            }
            else
            {
                if (v_prev_x < -float.Epsilon)
            ...
            if (direction_eq)
            {
                ...
                v_trans_x = -v_prev_y;
                v_trans_y = v_prev_x;
                shrink_by = (float)Math.Sqrt(v_prev_lensq);
            }
            else
            {
                ...
                v_trans_x = v_prev_x;
                v_trans_y = v_prev_y;
                shrink_by = (float)Math.Sqrt(v_prev_lensq / 2);
            }
        }
    ...
    {
        return vec.Clone().MultiplyScalar(size).Add(pt.ToVector2());
    }
    ...
    private void F4(int a, int b, int c, int d)
    {
        ...
        var nextIndex = verticesArray.Count / 3;
        var uvs = uvgen.GenerateSideWallUV(this, verticesArray, nextIndex - 6, nextIndex - 3, nextIndex - 2,
            nextIndex - 1);
