...
public class RenderableFace : RenderableObject
{
    ...
    public Vector3 normalModel = new();
    public List<Vector2> uvs = new() { Vector2.Zero(), Vector2.Zero(), Vector2.Zero() };
    public RenderableVertex v1 = new();
    public RenderableVertex v2 = new();
    public RenderableVertex v3 = new();
    public int vertexNormalsLength;
    public List<Vector3> vertexNormalsModel = new() { Vector3.Zero(), Vector3.Zero(), Vector3.Zero() };
}
...
public class RenderableLine : RenderableObject
{
    ...
    public RenderableVertex v1 = new();
    public RenderableVertex v2 = new();
    public List<Color> vertexColors = new() { Color.Hex(0x000000), Color.Hex(0x000000) };
}
...
public class RenderList
{
    ...
    public void ProjectVertex(RenderableVertex vertex)
    {
        ...
        positionScreen.Set(positionWorld.X, positionWorld.Y, positionWorld.Z, 1)
            .ApplyMatrix4(projector._viewProjectionMatrix);
    ...
    public void PushLine(int a, int b)
    {
        ...
        v1.positionScreen.Set(v1.position.X, v1.position.Y, v1.position.Z, 1)
            .ApplyMatrix4(projector._modelViewProjectionMatrix);
        v2.positionScreen.Set(v2.position.X, v2.position.Y, v2.position.Z, 1)
            .ApplyMatrix4(projector._modelViewProjectionMatrix);

        if (projector.ClipLine(v1.positionScreen, v2.positionScreen))
        {
            ...
            projector._line.z = Math.Max(v1.positionScreen.Z, v2.positionScreen.Z);
    ...
    public void PushTriangle(int a, int b, int c, Material material, List<int> arguments)
    {
        ...
        if (material.Side == Constants.DoubleSide || CheckBackfaceCulling(v1, v2, v3))
        {
            ...
            if (material.VertexColors) projector._face.color.FromArray(colors.ToArray(), a * 3);
...
{
    public Box3 _boundingBox = new();

    public Box3 _clipBox = new(new Vector3(-1, -1, -1), new Vector3(1, 1, 1));

    public Vector4 _clippedVertex1PositionScreen = new();
    public Vector4 _clippedVertex2PositionScreen = new();
    public RenderableFace _face;
    public int _faceCount;
    public List<RenderableFace> _facePool;
    public int _facePoolLength;

    public Frustum _frustum = new();
    public RenderableLine _line;
    public int _lineCount;
    public List<RenderableLine> _linePool;
    public int _linePoolLength;

    public Matrix4 _modelMatrix = new();
    public Matrix4 _modelViewProjectionMatrix = new();

    public Matrix3 _normalMatrix = new();
    public RenderableObject _object;
    public int _objectCount;
    public List<RenderableObject> _objectPool = new();
    public int _objectPoolLength;
    public List<Vector4> _points3 = new();
    ...
    public int _spritePoolLength;
    public Vector3 _vector3 = new();
    public Vector4 _vector4 = new();
    public RenderableVertex _vertex;
    public int _vertexCount;
    public List<RenderableVertex> _vertexPool;
    public int _vertexPoolLength;

    public Matrix4 _viewMatrix = new();
    public Matrix4 _viewProjectionMatrix = new();
    ...
    public void ProjectObject(Object3D object3D)
    {
        ...
        else if (object3D is Mesh || object3D is Line || object3D is Points)
        {
            ...
            if (object3D.FrustumCulled && _frustum.IntersectsObject(object3D) == false) return;
        ...
        {
            if (object3D.Material.Visible == false) return;
            if (object3D.FrustumCulled && _frustum.IntersectsSprite(object3D as Sprite) == false) return;
        ...
        for (var i = 0; i < children.Count; i++) ProjectObject(children[i]);
    }
