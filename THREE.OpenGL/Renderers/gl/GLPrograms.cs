...
public class GLPrograms
{
    ...
    public List<GLProgram> Programs = new();

    private GLRenderer renderer;

    private Dictionary<string, string> shaderIDs = new();

    public ShaderLib ShaderLib = Global.ShaderLib;

    private bool vertexTextures;

    public GLPrograms(GLRenderer renderer, GLCubeMap cubeMaps, GLExtensions extension, GLCapabilities capabilities,
        GLBindingStates bindingStates, GLClipping clipping)
    {
        ...
        parameterNames = new List<string>
        {
            ...
            "objectSpaceNormalMap", "tangentSpaceNormalMap", "clearcoatNormalMap", "displacementMap", "specularMap",
            "roughnessMap", "metalnessMap", "gradientMap",
            "alphaMap", "combine", "vertexColors", "vertexTangents", "vertexUvs", "uvsVertexOnly", "fog", "useFog",
            "fogExp2",
    ...
    private int GetMaxBones(SkinnedMesh skinnedMesh)
    {
        ...
        var nVertexMatrices = (int)Math.Floor((float)((nVertexUniforms - 20) / 4));

        var maxBones = Math.Min(nVertexMatrices, bones.Length);

        if (maxBones < bones.Length)
        {
            Trace.TraceWarning("THREE.Renderers.GLRenderer: Skeleton has" + bones.Length +
    ...
    {
        var encoding = Constants.LinearEncoding;
    ...
    public Hashtable GetParameters(Material material, GLLights lights, List<Light> shadows, Object3D scene,
        Object3D object3D)
    {
        ...
        var maxBones = object3D is SkinnedMesh ? GetMaxBones((SkinnedMesh)object3D) : 0;

        if (!string.IsNullOrEmpty(material.Precision))
        {
            precision = capabilities.GetMaxPrecision(material.Precision);

            if (!precision.Equals(material.Precision))
                Trace.TraceWarning("THREE.Renderers.gl.GLPrograms.GetParameters:" + material.Precision +
        ...
        parameters.Add("instancingColor",
            object3D is InstancedMesh && (object3D as InstancedMesh).InstanceColor != null);
        parameters.Add("supportsVertexTextures", vertexTextures);

        //parameters.Add("numMultiviewViews",numMultiviewViews);

        parameters.Add("outputEncoding",
            GetTextureEncodingFromMap(currentRenderTarget == null ? null : currentRenderTarget.Texture));

        parameters.Add("map", material.Map != null);

        parameters.Add("mapEncoding", GetTextureEncodingFromMap(material.Map));

        parameters.Add("matcap",
            material is MeshMatcapMaterial ? (material as MeshMatcapMaterial).Matcap != null : false);

        parameters.Add("matcapEncoding",
            material is MeshMatcapMaterial
                ? GetTextureEncodingFromMap((material as MeshMatcapMaterial).Matcap)
                : Constants.LinearEncoding);

        parameters.Add("envMap", envMap != null);

        parameters.Add("envMapMode", material.EnvMap != null ? material.EnvMap.Mapping : null);

        parameters.Add("envMapEncoding", GetTextureEncodingFromMap(material.EnvMap));

        parameters.Add("envMapCubeUV",
            material.EnvMap != null && (material.EnvMap.Mapping == Constants.CubeUVReflectionMapping ||
        ...
        parameters.Add("combine", material.Combine != 0 ? material.Combine : null);
        ...
        var vertexAlphas = material.VertexColors && object3D.Geometry != null && object3D.Geometry is BufferGeometry &&
                           (object3D.Geometry as BufferGeometry).Attributes.ContainsKey("color") &&
                           (object3D.Geometry as BufferGeometry).Attributes["color"] != null &&
                           ((object3D.Geometry as BufferGeometry).Attributes["color"] as IBufferAttribute).ItemSize ==
                           4;
        parameters.Add("vertexAlphas", vertexAlphas);

        parameters.Add("vertexUvs",
            material.Map != null || material.BumpMap != null || material.NormalMap != null ||
            material.SpecularMap != null || material.AlphaMap != null || material.EmissiveMap != null ||
            material.RoughnessMap != null || material.MetalnessMap != null || material.ClearcoatNormalMap != null);

        parameters.Add("uvsVertexOnly",
            !(material.Map != null || material.BumpMap != null || material.NormalMap != null ||
              material.SpecularMap != null || material.AlphaMap != null || material.EmissiveMap != null ||
              material.RoughnessMap != null || material.MetalnessMap != null || material.ClearcoatNormalMap != null) &&
        ...
        parameters.Add("numDirLights",
            lights.state["directional"] != null ? (lights.state["directional"] as GLUniform[]).Length : 0);

        parameters.Add("numPointLights",
            lights.state["point"] != null ? (lights.state["point"] as GLUniform[]).Length : 0);

        parameters.Add("numSpotLights",
            lights.state["spot"] != null ? (lights.state["spot"] as GLUniform[]).Length : 0);

        parameters.Add("numRectAreaLights",
            lights.state["rectArea"] != null ? (lights.state["rectArea"] as GLUniform[]).Length : 0);

        parameters.Add("numHemiLights",
            lights.state["hemi"] != null ? (lights.state["hemi"] as GLUniform[]).Length : 0);

        parameters.Add("numDirLightShadows",
            lights.state["directionalShadowMap"] != null
                ? (lights.state["directionalShadowMap"] as Texture[]).Length
                : 0);

        parameters.Add("numPointLightShadows",
            lights.state["pointShadowMap"] != null ? (lights.state["pointShadowMap"] as Texture[]).Length : 0);

        parameters.Add("numSpotLightShadows",
            lights.state["spotShadowMap"] != null ? (lights.state["spotShadowMap"] as Texture[]).Length : 0);
        ...
        parameters.Add("depthPacking",
            material is MeshDepthMaterial ? (material as MeshDepthMaterial).DepthPacking : 0);

        parameters.Add("indexOfAttributeName", material.IndexOAttributeName);

        parameters.Add("extensionDerivatives",
            material is ShaderMaterial && (material as ShaderMaterial).extensions.derivatives);

        parameters.Add("extensionFragDepth",
            material is ShaderMaterial && (material as ShaderMaterial).extensions.fragDepth);

        parameters.Add("extensionDrawBuffers",
            material is ShaderMaterial && (material as ShaderMaterial).extensions.drawBuffers);

        parameters.Add("extensionShaderTextureLOD",
            material is ShaderMaterial && (material as ShaderMaterial).extensions.shaderTextureLOD);
    ...
    public string getProgramCacheKey(Hashtable parameters)
    {
        ...
        if ((bool)parameters["isRawShaderMaterial"] == false)
            ...
            {
                var obj = parameters[parameterNames[i]];
                var parameterKey = obj != null ? obj.ToString() : "";
