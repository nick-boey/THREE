...
{
    public Hashtable InfluencesList = new();

    public float[] MorphInfluences = new float[8];

    public List<float[]> WorkInfluences = new();

    public GLMorphtargets()
    {
        for (var i = 0; i < 8; i++) WorkInfluences.Add(new float[2] { i, 0 });
    }
    ...
    public void Update(Object3D object3D, BufferGeometry geometry, Material material, GLProgram program)
    {
        ...
        if (!InfluencesList.ContainsKey(geometry.Id))
        {
            ...
            for (var i = 0; i < length; i++) influences.Add(new float[2] { i, 0 });
        ...
        influences.Sort(delegate(float[] a, float[] b) { return (int)(Math.Abs(b[1]) - Math.Abs(a[1])); });
        ...
        WorkInfluences.Sort(delegate(float[] a, float[] b) { return (int)(a[0] - b[0]); });

        var morphTargets = material.MorphTargets && geometry.MorphAttributes.ContainsKey("position")
            ? geometry.MorphAttributes["position"] as List<IBufferAttribute>
            : null;
        var morphNormals = material.MorphNormals && geometry.MorphAttributes.ContainsKey("normal")
            ? geometry.MorphAttributes["normal"] as List<IBufferAttribute>
            : null;
                ...
                if (influence != null)
                {
                    ...
                    if (index != int.MaxValue && value > 0)
                    {
                        if (morphTargets != null)
                        {
                            ...
                            else if (morphTargets[index] != null &&
                                     geometry.Attributes["morphTarget" + i] != morphTargets[index])
                                geometry.SetAttribute("morphTarget" + i, morphTargets[index]);
                        }
                        ...
                        if (morphNormals != null)
                        {
                            ...
                            else if (morphNormals != null &&
                                     geometry.Attributes["morphNormal" + i] != morphNormals[index])
                    ...
                    {
                        if (morphTargets != null && geometry.Attributes.ContainsKey("morphTarget" + i))
                            geometry.deleteAttribute("morphTarget" + i);

                        if (morphNormals != null && geometry.Attributes.ContainsKey("morphNormal" + i))
