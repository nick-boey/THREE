...
public class GLProgram : DisposableObject, IGLProgram
{
    ...
    private string deprecatedUnrollLoopPattern =
    ...
    public Hashtable Diagnostics = new();
    ...
    private string unrollLoopPattern =
    ...
    public GLProgram(GLRenderer renderer, string cacheKey, Hashtable parameters, GLBindingStates bindingStates)
    {
        ...
        var VertexShader = parameters["vertexShader"] as string;

        var FragmentShader = parameters["fragmentShader"] as string;
        ...
        var defines = parameters["defines"] as Hashtable;
        ...
        var gammaFactorDefine = renderer.GammaFactor > 0 ? renderer.GammaFactor : 1.0f;

        var customExtensions = (bool)parameters["isGL2"] ? new[] { "" } : GenerateExtensions(parameters);
        ...
        var versionString = (string)parameters["glslVersion"] != null
            ? "#version " + (string)parameters["glslVersion"] + "\n"
            : "";
        ...
        if ((bool)parameters["isRawShaderMaterial"])
        {
            ...
            var customExtensionsList = customExtensions.ToList();
        ...
        else
        {
            var prefixVertexs = new[]
            {
                ...
                (bool)parameters["instancing"] ? "#define USE_INSTANCING" : "",
                (bool)parameters["instancingColor"] ? "#define USE_INSTANCING_COLOR" : "",
                ...
                (bool)parameters["normalMap"] && (bool)parameters["objectSpaceNormalMap"]
                    ? "#define OBJECTSPACE_NORMALMAP"
                    : "",
                (bool)parameters["normalMap"] && (bool)parameters["tangentSpaceNormalMap"]
                ...
                (bool)parameters["displacementMap"] && (bool)parameters["supportsVertexTextures"]
                ...
                (bool)parameters["morphNormals"] && (bool)parameters["flatShading"] == false
                ...
                (bool)parameters["logarithmicDepthBuffer"] && (bool)parameters["renderExtensionFragDepth"]
            ...
            var prefixFragments = new[]
            {
                ...
                (float)parameters["alphaTest"] != 0
                    ? "#define ALPHATEST " + (float)parameters["alphaTest"] +
                      ((float)parameters["alphaTest"] % 1 > 0 ? "" : ".0")
                    : "", // add ".0" if integer
                ...
                (bool)parameters["logarithmicDepthBuffer"] ? "#define USE_LOGDEPTHBUF" : "",
                (bool)parameters["logarithmicDepthBuffer"] && (bool)parameters["rendererExtensionFragDepth"]
                    ? "#define USE_LOGDEPTHBUF_EXT"
                    : "",

                ((bool)parameters["extensionShaderTextureLOD"] || (bool)parameters["envMap"]) &&
                (bool)parameters["rendererExtensionShaderTextureLOD"]
                ...
                (int)parameters["toneMapping"] != Constants.NoToneMapping
                    ? renderer.ShaderLib.getChunk("tonemapping_pars_fragment")
                    : "", // this code is required here because it is used by the toneMapping() function defined below
                (int)parameters["toneMapping"] != Constants.NoToneMapping
                    ? GetToneMappingFunction("toneMapping", (int)parameters["toneMapping"])
                    : "",

                (bool)parameters["dithering"] ? "#define DITHERING" : "",

                renderer.ShaderLib.getChunk(
                ...
                parameters["map"] != null
                    ? GetTexelDecodingFunction("mapTexelToLinear", (int)parameters["mapEncoding"])
                    : "",
                parameters["matcap"] != null
                    ? GetTexelDecodingFunction("matcapTexelToLinear", (int)parameters["matcapEncoding"])
                    : "",
                parameters["envMap"] != null
                    ? GetTexelDecodingFunction("envMapTexelToLinear", (int)parameters["envMapEncoding"])
                    : "",
                parameters["emissiveMap"] != null
                    ? GetTexelDecodingFunction("emissiveMapTexelToLinear", (int)parameters["emissiveMapEncoding"])
                    : "",
                parameters["lightMap"] != null
                    ? GetTexelDecodingFunction("lightMapTexelToLinear", (int)parameters["lightMapEncoding"])
                    : "",
                GetTexelEncodingFunction("linearToOutputTexel", (int)parameters["outputEncoding"]),
                !customDefines.Contains("DEPTH_PACKING") && parameters["depthPacking"] != null
                    ? "#define DEPTH_PACKING " + (int)parameters["depthPacking"]
                    : "",
                "\n"
            };
        ...
        var glVertexShader = new GLShader();
        glVertexShader.SetShaderType(ShaderType.VertexShader, vertexGlsl);
        var glFragmentShader = new GLShader();
        ...
        if ((bool)renderer.Debug["checkShaderErrors"])
        {
            ...
            var fragmentLog = GL.GetShaderInfoLog(glFragmentShader.ShaderId);
            ...
            if (linkStatus == 0)
            {
                ...
                var errorMessage = string.Format(
                    "THREE.Renderers.gl.GLProgram: shader error {0}, GL.VALIDATE_STATUS {1}, GL.GetProgramInfoLog {2}",
            ...
            else if (string.IsNullOrEmpty(vertexLog) || string.IsNullOrEmpty(fragmentLog)) haveDiagnostics = false;
            //int length;
            //string source;
            ...
            if (haveDiagnostics)
            {
                Diagnostics.Add("runnable", runnable);
                Diagnostics.Add("programLog", programLog);
                Diagnostics.Add("vertexShader", new Hashtable { { "log", vertexLog }, { "prefix", prefixVertex } });
                Diagnostics.Add("fragmentShader",
                    new Hashtable { { "log", fragmentLog }, { "prefix", prefixFragment } });
            }
        }
    ...
    {
        var lines = shaderSource.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
        var newSource = string.Empty;

        foreach (var line in lines) newSource += line + Environment.NewLine;
    ...
    private string GetShaderErrors(GLShader shader, string type)
    {
        ...
        var log = GL.GetShaderInfoLog(shader.ShaderId);
    ...
    private string[] GenerateExtensions(Hashtable parameters)
    {
        ...
        if ((bool)parameters["extensionDerivatives"] || (bool)parameters["envMapCubeUV"] ||
            (bool)parameters["bumpMap"] || (bool)parameters["tangentSpaceNormalMap"] ||
            (bool)parameters["clearcoatNormalMap"] || (bool)parameters["flatShading"])
            chunks[0] = "#extension GL_OES_standard_derivatives : enable";
        else
            chunks[0] = "";

        if (((bool)parameters["extensionFragDepth"] || (bool)parameters["logarithmicDepthBuffer"]) &&
            (bool)parameters["renderExtensionFragDepth"])
        ...
        if (((bool)parameters["extensionShaderTextureLOD"] || (bool)parameters["envMap"] ||
             (bool)parameters["transmission"]) && parameters.ContainsKey("renderExtensionShaderTextureLod") &&
            (bool)parameters["renderExtensionShaderTextureLod"])
    ...
    private Hashtable FetchAttributeLocations(int program)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            var name = GL.GetActiveAttrib(program, i, out size, out attrType);
    ...
    private string ReplaceClippingPlaneNums(string str, Hashtable parameters)
    {
        return str
            ...
            .Replace("UNION_CLIPPING_PLANES",
                ((int)parameters["numClippingPlanes"] - (int)parameters["numClipIntersection"]).ToString());
    }
    ...
    private string IncludeReplacer(Match match)
    {
        ...
        if (match.Groups.Count > 1)
            ...
            {
                var include = match.Groups[ctr].Value;
                var source = renderer.ShaderLib.getChunk(include);
                if (string.IsNullOrEmpty(source)) throw new Exception("Can not resolve #include<" + include + ">");
    ...
    private string LoopReplacer(Match match)
    {
        ...
        if (match.Groups.Count > 1)
        {
            ...
            var snippet = match.Groups[3].Value;
            if (start == 0 && end == 0)
                result = "";
            else
    ...
    private string GeneratePrecision(Hashtable parameters)
    {
        ...
        else if (parameters["precision"].Equals("lowp")) precisionstring += @"#define LOW_PRECISION";
    ...
    private string GenerateShadowMapTypeDefine(Hashtable parameters)
    {
        ...
        else if ((int)parameters["shadowMapType"] == Constants.VSMShadowMap) shadowMapTypeDefine = "SHADOWMAP_TYPE_VSM";
    ...
    private string GenerateEnvMapTypeDefine(Hashtable parameters)
    {
        ...
        {
            var envMapMode = (int)parameters["envMapMode"];
            switch (envMapMode)
            {
    ...
    private string GenerateEnvMapBlendingDefine(Hashtable parameters)
    {
        ...
        if ((bool)parameters["envMap"])
        {
            var combine = (int?)parameters["combine"];
            if (combine != null)
                switch (combine)
                {
