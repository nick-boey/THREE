...
public class GLTextures : DisposableObject, IGLTextures
{
    ...
    private List<int> filterToGL = new()
    {
        (int)All.Nearest,
    ...
    private Hashtable videoTextures = new();

    private bool warnedTexture2D = false;
    private bool warnedTextureCube;

    public GLTextures(IGraphicsContext context, GLExtensions extensions, GLState state, GLProperties properties,
        GLCapabilities capabilities, GLUtils util, GLInfo info)
    {
        Context = context;
        ...
        maxSample = capabilities.maxSamples;
    ...
    public int AllocateTextureUnit()
    {
        ...
        if (textureUnit >= maxTextures)
            Trace.TraceWarning("THREE.GLTextures: Trying to use " + textureUnits +
    ...
    private SKBitmap ResizeImage(SKBitmap image, bool needsPowerOfTwo, bool needsNewCanvas, int maxSize)
    {
        ...
        if (image.Width > maxSize || image.Height > maxSize)
            scale = maxSize / (float)Math.Max(image.Width, image.Height);

        if (scale <= 1 || needsPowerOfTwo)
        {
            if (image is SKBitmap)
            {
                var width = needsPowerOfTwo
                    ? MathUtils.FloorPowerOfTwo(scale * image.Width)
                    : (float)Math.Floor(scale * image.Width);
                var height = needsPowerOfTwo
                    ? MathUtils.FloorPowerOfTwo(scale * image.Height)
                    : (float)Math.Floor(scale * image.Height);
    ...
    {
        return texture.GenerateMipmaps && supportsMips && texture.MinFilter != Constants.NearestFilter &&
    ...
    private void GenerateMipmap(TextureTarget target, Texture texture, int width, int height)
    {
        ...
        var value = Math.Log(Math.Max(width, height)) * Math.Log(Math.E, 2);
    ...
    private int GetInternalFormat(string internalFormatName, int glFormat, int glType)
    {
        ...
        if (internalFormat == (int)All.R16f || internalFormat == (int)All.R32f ||
            ...
            Trace.TraceWarning(
                "THREE.GLRenderer: Floating point textures with RGB format not supported. Please use RGBA instead.");
    ...
    {
        if (f == Constants.NearestFilter || f == Constants.NearestMipmapNearestFilter ||
            f == Constants.NearestMipmapLinearFilter) return (int)All.Nearest;
    ...
    public void DeallocateRenderTarget(GLRenderTarget renderTarget)
    {
        ...
        if (textureProperties.ContainsKey("glTexture")) GL.DeleteTexture((int)textureProperties["glTexture"]);

        if (renderTarget.depthTexture != null) renderTarget.depthTexture.Dispose();

        if (renderTarget is GLCubeRenderTarget)
        {
            var bufferList = (int[])renderTargetProperties["glFramebuffer"];
            var depthList = (int[])renderTargetProperties["glDepthbuffer"];
            for (var i = 0; i < 6; i++)
            {
        ...
        else
        {
            ...
            if (renderTargetProperties.ContainsKey("glDepthbuffer"))
                GL.DeleteRenderbuffer((int)renderTargetProperties["glDepthbuffer"]);
        }
        ...
        if (renderTarget is GLMultiviewRenderTarget)
        {
            ...
            var framebuffers = (int[])renderTargetProperties["glViewFramebuffers"];

            for (var i = 0; i < framebuffers.Length; i++) GL.DeleteFramebuffer(framebuffers[i]);
        }
    ...
    public void SetTexture2D(Texture texture, int slot)
    {
        ...
        var textureVersion = textureProperties.ContainsKey("version") ? (int)textureProperties["version"] : -1;
        ...
        if (texture.version > 0 && textureVersion != texture.version)
        {
            if (texture.Image == null && texture.ImageSize.Width > 0 && texture.ImageSize.Height > 0 &&
                !(texture is DataTexture))
            {
                var data = new byte[texture.ImageSize.Width * texture.ImageSize.Height * 4];
            ...
            {
                Trace.TraceWarning(
                    "THREE.Renderers.gl.GLTextures.SetTexture2D : Texture marked for update but image is undefined");
            }
            else
            {
                UploadTexture(textureProperties, texture, slot);
    ...
    public void SetTextureCube(Texture texture, int slot)
    {
        ...
        if (texture.version > 0 && textureVersion != texture.version)
        {
            ...
            state.ActiveTexture((int)TextureUnit.Texture0 + slot);
            var textureId = (int)textureProperties["glTexture"];
            ...
            var supportsMipsMap =
                image is SKBitmap ? IsPowerOfTwo((SKBitmap)image) : IsPowerOfTwo((image as Texture).Image);
            var supportsMips = supportsMipsMap ? supportsMipsMap : IsGL2;

            var glFormat = utils.Convert(texture.Format);
            var glType = utils.Convert(texture.Type);
            var glInternalFormat = GetInternalFormat(texture.InternalFormat, (int)glFormat, (int)glType);

            SetTextureParameters(TextureTarget.TextureCubeMap, texture, supportsMips);

            var targets = new List<TextureTarget2d>
            {
                ...
                for (var i = 0; i < 6; i++)
                {
                    ...
                    for (var j = 0; j < mipmaps.Count; j++)
                    {
                        ...
                        if (texture.Format != Constants.RGBAFormat && texture.Format != Constants.RGBFormat)
                            //if ( glFormat != null ) {
                            state.CompressedTexImage2D((int)targets[i], j, glInternalFormat, mipmap.Width,
                                mipmap.Height, 0, mipmap.Data);
                        //} else {
                        //    console.warn( 'THREE.WebGLRenderer: Attempt to load unsupported compressed texture format in .setTextureCube()' );
                        //}
                        else
                            state.TexImage2D((int)targets[i], j, glInternalFormat, mipmap.Width, mipmap.Height, 0,
                                (int)glFormat, (int)glType, mipmap.Data);
                    }
                }
            ...
            else
            {
                ...
                for (var i = 0; i < 6; i++)
                {
                    var localImage = cubeImage[i] as SKBitmap;
                    if (isDataTexture)
                    {
                        state.TexImage2D((int)targets[i], 0, glInternalFormat, localImage.Width, localImage.Height, 0,
                            (int)glFormat, (int)glType, localImage.Bytes);
                        ...
                        for (var j = 0; j < mipmaps.Count; j++)
                        {
                            ...
                            state.TexImage2D((int)targets[i], j + 1, glInternalFormat, mipmap.Width, mipmap.Height, 0,
                                (int)glFormat, (int)glType, mipmapImage);
                        }
                    }
                    else
                    {
                        ...
                        GL.TexImage2D((All)targets[i], 0, (All)glInternalFormat, localImage.Width, localImage.Height, 0,
                            All.BgraImg, glType, localImage.Bytes);
            ...
            if (TextureNeedsGenerateMipmaps(texture, supportsMips))
                ...
                GenerateMipmap(TextureTarget.TextureCubeMap, texture, (image as SKBitmap).Width,
                    (image as SKBitmap).Height);
    ...
    private int WrappingToGL(int wrap)
    {
        ...
        if (Constants.ClampToEdgeWrapping == wrap)
            return (int)All.ClampToEdge;
        if (Constants.MirroredRepeatWrapping == wrap)
    ...
    private int FilterToGL(int filter)
    {
        ...
        if (Constants.NearestMipmapNearestFilter == filter)
            return (int)All.NearestMipmapNearest;

        if (Constants.NearestMipmapLinearFilter == filter)
            return (int)All.NearestMipmapLinear;

        if (Constants.LinearFilter == filter)
            return (int)All.Linear;

        if (Constants.LinearMipmapNearestFilter == filter)
            return (int)All.LinearMipmapNearest;

        if (Constants.LinearMipmapLinearFilter == filter)
    ...
    private void SetTextureParameters(TextureTarget textureType, Texture texture, bool supportsMips)
    {
        ...
        else
        {
            ...
            if (texture.WrapS != Constants.ClampToEdgeWrapping || texture.WrapT != Constants.ClampToEdgeWrapping)
                Trace.TraceWarning(
                    "THREE.GLRenderer: Texture is not power of two. Texture.wrapS and Texture.wrapT should be set to THREE.ClampToEdgeWrapping.");
            ...
            if (texture.MinFilter != Constants.NearestFilter && texture.MinFilter != Constants.LinearFilter)
                Trace.TraceWarning(
                    "THREE.GLRenderer: Texture is not power of two. Texture.minFilter should be set to THREE.NearestFilter or THREE.LinearFilter.");
        }
        ...
        if (extension > -1)
        {
            ...
            if (texture.Type == Constants.HalfFloatType &&
                (IsGL2 || extensions.Get("GL_OES_texture_half_float_linear") == -1)) return;

            if (texture.Anisotropy > 1 || properties.Get(texture)["currentAnisotropy"] != null)
            {
                GL.TexParameter(textureType,
                    (TextureParameterName)ExtTextureFilterAnisotropic.MaxTextureMaxAnisotropyExt,
    ...
    private void UploadTexture(Hashtable textureProperties, Texture texture, int slot)
    {
        ...
        var supportsMips = IsPowerOfTwo(image) ? true : IsGL2;
        var glFormat = utils.Convert(texture.Format);
        var glType = utils.Convert(texture.Type);
        var glInternalFormat = GetInternalFormat(texture.InternalFormat, (int)glFormat, (int)glType);
        ...
        if (texture is DepthTexture)
        {
            ...
            state.TexImage2D((int)TextureTarget2d.Texture2D, 0, glInternalFormat, image.Width, image.Height, 0,
                (int)glFormat, (int)glType, null);
        }
        else if (texture is DataTexture)
        {
            ...
            if (mipmaps.Count > 0 && supportsMips)
            {
                ...
                {
                    mipmap = mipmaps[i];
                    state.TexImage2D((int)TextureTarget2d.Texture2D, i, glInternalFormat, mipmap.Width, mipmap.Height,
                        0, (int)glFormat, (int)glType, mipmap.Data);
                }
            ...
            else
            {
                ...
                if (texture.Image != null)
                {
                    ...
                    GL.TexImage2D(All.Texture2D, 0, (All)glInternalFormat, image.Width, image.Height, 0, All.BgraImg,
                        glType, data.ToSKBitMap(image.Width, image.Height).Bytes);

                    textureProperties["maxMipLevel"] = 0;
                }
                else
                {
                    if ((texture as DataTexture).byteData != null || (texture as DataTexture).intData != null ||
                        (texture as DataTexture).floatData != null)
                        switch ((texture as DataTexture).Type)
                        {
                            case 1015: // Constants.FloatType:
                                GL.TexImage2D(TextureTarget2d.Texture2D, 0, (TextureComponentCount)glInternalFormat,
                                    (texture as DataTexture).ImageSize.Width, (texture as DataTexture).ImageSize.Height,
                                    0, PixelFormat.Rgba, (PixelType)glType, (texture as DataTexture).floatData);
                                textureProperties["maxMipLevel"] = 0;
                                break;
                            case 1013: //Constants.IntType:
                                GL.TexImage2D(TextureTarget2d.Texture2D, 0, (TextureComponentCount)glInternalFormat,
                                    (texture as DataTexture).ImageSize.Width, (texture as DataTexture).ImageSize.Height,
                                    0, PixelFormat.Rgba, (PixelType)glType, (texture as DataTexture).intData);
                                textureProperties["maxMipLevel"] = 0;
                                break;
                            case 1010: //Constants.ByteType:
                                GL.TexImage2D(TextureTarget2d.Texture2D, 0, (TextureComponentCount)glInternalFormat,
                                    (texture as DataTexture).ImageSize.Width, (texture as DataTexture).ImageSize.Height,
                                    0, PixelFormat.Rgba, (PixelType)glType, (texture as DataTexture).byteData);
                                textureProperties["maxMipLevel"] = 0;
                                break;
                        }
                }
        ...
        else if (texture is CompressedTexture)
        {
            for (var i = 0; i <= mipmaps.Count; i++)
            {
                ...
                {
                    if (glFormat != null)
                        state.CompressedTexImage2D((int)TextureTarget2d.Texture2D, i, glInternalFormat, mipmap.Width,
                            mipmap.Height, 0, mipmap.Data);
                    else
                        Trace.TraceWarning(
                            "THREE.GLRenderer: Attempt to load unsupported compressed texture format in .uploadTexture()");
                }
                else
                {
                    state.TexImage2D((int)TextureTarget2d.Texture2D, i, glInternalFormat, mipmap.Width, mipmap.Height,
        ...
        {
            state.TexImage3D((int)All.Texture2DArray, 0, glInternalFormat, (texture as DataTexture2DArray).Width,
                (texture as DataTexture2DArray).Height, (texture as DataTexture2DArray).Depth, 0, (int)glFormat,
                (int)glType, (texture as DataTexture2DArray).Data);
            textureProperties["maxMipLevel"] = 0;
        }
        else if (texture is DataTexture3D)
        {
            state.TexImage3D((int)All.Texture3D, 0, glInternalFormat, (texture as DataTexture3D).Width,
                (texture as DataTexture3D).Height, (texture as DataTexture3D).Depth, 0, (int)glFormat, (int)glType,
                (texture as DataTexture3D).Data);
            textureProperties["maxMipLevel"] = 0;
        }
        else
        {
    ...
    public void SetupFrameBufferTexture(int framebuffer, GLRenderTarget renderTarget, All attachment, All textureTarget)
    {
        ...
        var target = (TextureTarget2d)textureTarget;
        state.TexImage2D((int)target, 0, glInternalFormat, renderTarget.Width, renderTarget.Height, 0, (int)glFormat,
            (int)glType, IntPtr.Zero);
        var texture = (int)properties.Get(renderTarget.Texture)["glTexture"];
        GL.BindFramebuffer(FramebufferTarget.Framebuffer, framebuffer);
        GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, (FramebufferAttachment)attachment,
            (TextureTarget2d)textureTarget, texture, 0);
    ...
    public void SetupRenderBufferStorage(int renderbuffer, GLRenderTarget renderTarget, bool isMultisample)
    {
        ...
        if (renderTarget.depthBuffer && !renderTarget.stencilBuffer)
        {
            if (isMultisample)
            {
                ...
                GL.RenderbufferStorageMultisample(RenderbufferTarget.Renderbuffer, samples,
                    RenderbufferInternalFormat.DepthComponent16, renderTarget.Width, renderTarget.Height);
            }
            else
            {
        ...
        else if (renderTarget.depthBuffer && renderTarget.stencilBuffer)
        {
            if (isMultisample)
            {
                ...
                GL.RenderbufferStorageMultisample(RenderbufferTarget.Renderbuffer, samples,
                    RenderbufferInternalFormat.Depth24Stencil8, renderTarget.Width, renderTarget.Height);
            }
            else
            {
                GL.RenderbufferStorage(RenderbufferTarget.Renderbuffer, RenderbufferInternalFormat.Depth24Stencil8,
                    renderTarget.Width, renderTarget.Height);
            }
        ...
        else
        {
            ...
            if (isMultisample)
            {
                ...
                GL.RenderbufferStorageMultisample(RenderbufferTarget.Renderbuffer, samples,
                    (RenderbufferInternalFormat)glInternalFormat, renderTarget.Width, renderTarget.Height);
            }
            else
            {
                GL.RenderbufferStorage(RenderbufferTarget.Renderbuffer, (RenderbufferInternalFormat)glInternalFormat,
                    renderTarget.Width, renderTarget.Height);
            }
        }
    ...
    public void SetupDepthTexture(int framebuffer, GLRenderTarget renderTarget)
    {
        ...
        if (renderTarget.depthTexture.Format == Constants.DepthFormat)
            GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.DepthAttachment,
                TextureTarget2d.Texture2D, glDepthTexture, 0);
    ...
    public void SetupDepthRenderbuffer(GLRenderTarget renderTarget)
    {
        ...
        else
        {
            if (isCube)
            {
                var depthbuffer = new int[6];
                var framebuffer = (int[])renderTargetProperties["glFramebuffer"];
                for (var i = 0; i < 6; i++)
                {
    ...
    public void SetupRenderTarget(GLRenderTarget renderTarget)
    {
            ...
            if (isMultisample)
            {
                if (IsGL2)
                {
                    ...
                    GL.BindRenderbuffer(RenderbufferTarget.Renderbuffer,
                        (int)renderTargetProperties["glColorRenderbuffer"]);
                    var glFormat = utils.Convert(renderTarget.Texture.Format);
                    var glType = utils.Convert(renderTarget.Texture.Type);
                    var glInternalFormat =
                        GetInternalFormat(renderTarget.Texture.InternalFormat, (int)glFormat, (int)glType);
                    var samples = GetRenderTargetSamples(renderTarget);
                    GL.RenderbufferStorageMultisample(RenderbufferTarget.Renderbuffer, samples,
                        (RenderbufferInternalFormat)glInternalFormat, renderTarget.Width, renderTarget.Height);

                    GL.BindFramebuffer(FramebufferTarget.Framebuffer,
                        (int)renderTargetProperties["glMultisampledFramebuffer"]);
                    GL.FramebufferRenderbuffer(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0,
                        RenderbufferTarget.Renderbuffer, (int)renderTargetProperties["glColorRenderbuffer"]);
                    GL.BindRenderbuffer(RenderbufferTarget.Renderbuffer, 0);

                    if (renderTarget.depthBuffer)
                    {
                        renderTargetProperties["glDepthRenderbuffer"] = GL.GenRenderbuffer();
                        SetupRenderBufferStorage((int)renderTargetProperties["glDepthRenderbuffer"], renderTarget,
                            true);
                    }
        ...
        if (isCube)
        {
            ...
            for (var i = 0; i < 6; i++)
                SetupFrameBufferTexture(((int[])renderTargetProperties["glFramebuffer"])[i], renderTarget,
                    All.ColorAttachment0, All.TextureCubeMapPositiveX + i);

            if (TextureNeedsGenerateMipmaps(renderTarget.Texture, supportsMips))
                GenerateMipmap(TextureTarget.TextureCubeMap, renderTarget.Texture, renderTarget.Width,
                    renderTarget.Height);
        ...
        else
        {
            ...
            SetupFrameBufferTexture((int)renderTargetProperties["glFramebuffer"], renderTarget, All.ColorAttachment0,
                All.Texture2D);
    ...
    public void UpdateRenderTargetMipmap(GLRenderTarget renderTarget)
    {
        ...
        var supportsMips = IsPowerOfTwo(renderTarget.Image) || IsGL2;

        if (TextureNeedsGenerateMipmaps(texture, supportsMips))
        {
            var target = renderTarget is GLCubeRenderTarget ? TextureTarget.TextureCubeMap : TextureTarget.Texture2D;
            var glTexture = (int?)properties.Get(texture)["glTexture"];
    ...
    public void UpdateMultisampleRenderTarget(GLRenderTarget renderTarget)
    {
        if (renderTarget is GLMultisampleRenderTarget)
        {
            ...
            {
                Trace.TraceWarning(
                    "THREE.Renderers.gl.GLTextures : GLMultisampleRenderTarget can only be used with GL2.");
            }
        }
    }
    ...
    {
        return IsGL2 && renderTarget is GLMultisampleRenderTarget
            ? Math.Min(maxSample, (renderTarget as GLMultisampleRenderTarget).Samples)
            : 0;
    }
