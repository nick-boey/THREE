...
public class SingleUniform : GLUniform, ISingleUniform
{
    ...
    public void SetValue(Vector4 v)
    {
        ...
        GL.Uniform4(Addr, v.X, v.Y, v.Z, v.W);
    ...
    public void SetValue(Matrix3 matrix)
    {
        ...
        GL.UniformMatrix3(Addr, 1, false, matrix.Elements);
    ...
    public void SetValue(Matrix4 matrix)
    {
        ...
        GL.UniformMatrix4(Addr, 1, false, matrix.Elements);
    ...
    public void SetValue(Color color)
    {
        ...
        GL.Uniform3(Addr, color.R, color.G, color.B);
    ...
    public void SetValue(float[] color)
    {
        ...
        GL.Uniform3(Addr, color[0], color[1], color[2]);
    ...
    public void SetValue(object v, IGLTextures textures = null)
    {
        ...
        else if (UniformType == (int)ActiveUniformType.Int)
        {
            if (v is bool)
            {
                var value = Convert.ToBoolean(v);
                SetValue(value ? 1 : 0);
                UniformType = (int)ActiveUniformType.Int;
            }
            else
            {
                SetValue((int)v);
        ...
        else if (UniformType == (int)ActiveUniformType.Bool)
        {
            var value = Convert.ToBoolean(v);
            SetValue(value ? 1 : 0);
            UniformType = (int)ActiveUniformType.Int;
        }
        else if (UniformType == (int)ActiveUniformType.Float)
        {
        ...
        {
            Trace.TraceWarning("SingleUniform not defined value was access " + v);
        }
    }
}