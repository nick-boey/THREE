...
public class TrackballControls
{
    ...
    private Vector4 screen = Vector4.Zero();
    ...
    public TrackballControls(IControlsContainer control, Camera camera)
    {
        ...
        screen = new Vector4(0, 0, control.ClientRectangle.Width, control.ClientRectangle.Height);
    ...
    private void RotateCamera()
    {
        ...
        var angle = (float)Math.Acos(rotateStart.Dot(rotateEnd) / rotateStart.Length() / rotateEnd.Length());
    ...
    private void ZoomCamera()
    {
        ...
        else
        {
            ...
            if (factor != 1.0 && factor > 0.0f) eye.MultiplyScalar(factor);
            if (StaticMoving)
    ...
    public void Update()
    {
        ...
        if (!NoRotate) RotateCamera();

        if (!NoZoom) ZoomCamera();
    ...
    {
        screen.Set(0, 0, e.Width, e.Height);
    ...
    public void Control_MouseMove(object sender, MouseEventArgs e)
    {
        ...
        else if (state == STATE.PAN && !NoPan) panEnd = GetMouseOnScreen(e.X, e.Y);
    }
